...
| F06 | **`lld-ready` gate** — deterministic (0 LLM), Rust: refuses a freeze with any open question, no owner, no acceptance line, or a `depth_score` below a threshold calibrated against a labeled corpus (the depth-bar feedback: under-depth is a gate failure, not a judgment call) | fleet | F02 | lld.v1 contract - PASS, merged, found+fixed 2 more cross-lang bugs | fleet+orb | ☑ |
