...
| F09 | **Orb→Fleet handoff wiring** — the ❄ freeze event actually calls fleet's extended intake (F07); no more "stop at a frozen `lld.v1` artifact" (the prior session's Track B boundary) | orb+fleet | F05, F07 | build-new (the seam) | Freezing a real module in a live orb build-mode session results in a real fleet SOW being created, observably (not just a documented intent) | ☑ PASS, merged (keel would accept a hand-forged freeze — disclosed `mitigates`; orb's own code never constructs one, proven) |
| F10 | **P0 capstone: one module, spoken → frozen → built → attested → PR, no hand-written spec** — integration proof of F01–F09 together, on the reused fleet-rs kernel (`run_with_evidence`, unchanged). **Scope measured 2026-09-07 (contract §3.1): 4 of the 7 hops are already automatic in merged code; SOW-accept is manual *by law* (A15/D4, `main.rs:665-722`) and stays so; only `accepted SOW → run_with_evidence` and `Accepted → pr emit` are unwired — nothing in the tree drives them.** So the build-new scope is one ~120-line shell driver over two existing CLI commands, plus the suite: **zero Rust, zero Python** (gated: `git diff -- fleet/keel orb/backend/relay-py` must be empty) | orb+fleet | F01–F09 | reuse (kernel) + integrate (driver only) | Drive one real, small module through the full path and watch a real PR appear with a real attestation attached — this is the P0 exit trigger, measured wall-clock freeze→attested-PR. **Two tiers: Tier A (`--agent stub`, gated in `verify.sh`) proves the chain is wired; Tier B (real gateway + real builder agent + real `gh`) IS the exit trigger. Tier A green alone does not close F10** (`proxy-is-not-the-property`) | ☐ |
...
| F37 | Two-pane chat UI shell — native SwiftUI left pane rendering **real** conversation turns from a live relay stack. **Row corrected 2026-09-07 by the F37 lead:** the previous wording ("app shell + orb visual state already built", branch `extend`) was wrong for `Light/` — the orb visual + relay socket/codec ARE installed, but there is **no app shell and no demo mock** in `Light/apps/macos` (`ContentView` is 33 lines rendering one orb; the upstream `ChatViewModel`/`AppShellView` types were never copied into Light). See `docs/lane-contracts/F37-two-pane-chat-ui.md` | apps/macos | F01, F09 | build-new (chat pane + Swift HTTP client for `/v1/respond`); installs: Foundation `URLSessionWebSocketTask`, `RelaySocket` lifecycle, `WireProtocol` codec, `stub_gateway.py` | Real turns from a live `relay-rs`+`relay-py`+stub-gateway stack render in the left pane in **build mode**, asserted on **rasterised pixels** (in-process `NSHostingView`+Vision OCR — external capture is TCC-denied on this host), with a two-run parameterised control that a hardcoded string cannot pass | ☐ contract written |
