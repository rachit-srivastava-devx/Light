...
| F03 | **Module-brief atomizer** — `lld_decomposer.py` already exists (merged) but has zero production callers; wire it into `mode=build`'s `/v1/respond` branch (F04's existing seam), plus fix the blind-repair defect (switch to `validate_module_brief_detailed`) | orb (`backend/relay-py`) | F02, F04 | install + wire | A build-mode request produces a real `ModuleBrief` via the wired path; focus-mode provably untouched | ◐ contract done |
...
| F06 | **`lld-ready` gate**, fleet-only (extract 3 predicates already ported in `lld.rs` + build-new the rest: reachable MeasuredNothing, stamping, comparator). Found 2 real defects in F02's already-merged TS gate: unreachable dead code, a coverage test sold as a census covering only 8/14 checks | fleet | F02 | extract+build-new | 14/14 checks fire on the control fixture, 0-checked is a hard refusal not a pass | ◐ contract done |
...
| F35 | Contract-codegen pipeline — keel JSON-Schema → Swift `Codable`, CI-gated against staleness (greenfield on all sides, not an existing pattern — see `18-THE-LIGHT-APP-AND-UI.md` §3.4's own correction) | apps/macos+fleet | F02 | lld.v1 contract - PASS, merged, found+fixed 2 more cross-lang bugs | fleet+orb | ☑ |
