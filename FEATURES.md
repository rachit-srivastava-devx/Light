...
| F02 | **`lld.v1` contract** — schema (purpose, interface, acceptance_line, deps[], killed_alternatives[] ≥2, owner; gate-stamped: frozen_at, c1_verdict, depth_score) + TS mirror (orb) + Rust/Python mirrors (fleet), all validated against shared fixtures (learn from the prior session's U1 unit: SHA-256 canonical-JSON, the numeric-encoding trap between `JSON.stringify`/`json.dumps`) | fleet (schema owner) + orb (consumer) | S0 | extract (C2) + build-new — the D15 copy already carried a schema + TS/Python mirror across (U1), just under the wrong repo/name with 4 real defects (see the F02 lane contract §2); only the Rust mirror, the hand-off wrapper, and the cross-language comparator are genuinely new | Same fixture round-trips through all 3 language mirrors byte-identical; a fixture missing a required field fails validation in all 3 | ☐ |
| F03 | **Module-brief atomizer** — repoint the existing atomizer's output type from physical ADHD-task steps to module briefs (interface/data-owned/acceptance/deps/non-goals), keeping the structured-decode→validate→repair-once→fail-closed pipeline shape. Already correctly scoped server-side | orb (`backend/relay-py/src/orb_relay/proxy/atomizer.py` — already backend) | F02 | extend | A spoken module idea produces a schema-valid module-brief object (not a physical-step list) via the existing pipeline; the old focus-mode atomizer path is untouched | ☐ |
| F04 | **REDEFINED 2026-09-07 (was client-side `BeliefModel.ts` only) — server-side build-mode session + depth-completeness belief registers, in `relay-py`.** Closes F01's reachability gap: build-mode session state (which mode a session is in, the build-specific greeting/system-prompt) and the re-aimed belief math (spec-completeness/ambiguity/coverage log-odds registers, same κ/decay law as the client's `BeliefModel.ts`, ported to Python) both live in `backend/relay-py`, driven over the existing relay protocol — so `apps/macos` (Swift) *and* the old RN client can both select build mode and see belief-driven behavior with zero client-side FSM/belief copy | orb (`backend/relay-py` — NEW target, reachable by both clients) | S0 | build-new (server-side; port the client math, don't re-derive it) | A session opened with `mode=build` over the relay (from any client, including a raw WebSocket test client with no RN/Swift involved) gets a build-specific greeting and a real completeness register that moves on scripted evidence; existing focus-mode relay behavior is unchanged | ☐ |
| F05 | **Freeze protocol**, server-side (`relay-py`) — propose→pushback→❄ conversational format computed in the backend and sent over the relay as turn data; new slot set (interface/data-owned/acceptance/deps/non-goals); converge bounded by information gain, not the old fixed `MAX_CLARIFY_QUESTIONS=2` (client's `ClarifyProtocol.ts` is the reference for the math/rules, not the new home for the code) | orb (`backend/relay-py` — not `apps/mobile/src/lld/ClarifyProtocol.ts`) | F02, F03, F04 | build-new (new bound; keep rule-driven-no-LLM-decides-the-slot design; port to Python) | A scripted conversation with all 5 slots answered reaches a ❄ freeze in a bounded, provably-terminating number of turns, driven over the relay from a raw test client; one with a slot deliberately never answered does **not** freeze and does not loop forever | ☐ |
| F06 | **`lld-ready` gate** — deterministic (0 LLM), Rust: refuses a freeze with any open question, no owner, no acceptance line, or a `depth_score` below a threshold calibrated against a labeled corpus (the depth-bar feedback: under-depth is a gate failure, not a judgment call). **Shipped diverges from this sentence on purpose (F06 lane contract §1.1, decided at the lead level): there is no threshold — the bar is the full 14-check conjunction (any one real failure refuses, a 13/14 brief included), and `depth_score` is reported, never a decision input. Calibrating a threshold against a labeled corpus is real work but is F17's, not F06's — no labeled set is scored in this tree yet.** | fleet | F02 | extract (C2) — port `LldReadyGate.ts`'s 14 checks id-for-id + reuse `lld.rs`'s already-comparator-verified predicates (`valid_node_id`/`number_calc_ok`/`structural_enforced_by_ok`, made `pub(crate)`, zero logic change); build-new only the reachable-`MeasuredNothing` seam, `gate-refs.v1.json`, the freeze-stamping path, `fleet gate lld-ready` CLI dispatch, and the Rust↔TS comparator | Shipped as `fleet gate lld-ready`'s 14-check conjunction (F06-T1..T10, `fleet/keel/fleet/tests/f06_lld_ready.rs`) plus a 2-direction selftest and a Rust↔TS comparator; a brief missing `alternatives`/failing any other check is refused by name; the control fixture (14/14) passes; no threshold exists to calibrate (see this row's divergence note) — that is F17's | ☐ |
