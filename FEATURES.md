...
| F17 | **Depth-bar tightening** — the `lld-ready` rubric gains eval/threshold checks (a freeze claiming a rate must carry sample-size math) | orb+fleet | F06 | lld-ready gate - PASS, merged | fleet | ☑ |
...
| F27 | `planner` role (D8) — 6th fleet role, optional per module | fleet | F06 | lld-ready gate - PASS, merged | fleet | ☑ |
...
| F33 | D10 trigger mechanism — explicit-freeze-or-fleet-decides, a confidence-score proposer + deterministic threshold gate (not a free-floating auto-trigger) | fleet | F06 | lld-ready gate - PASS, merged | fleet | ☑ |
