...
use super::render;
use crate::render_event::{Event, Outcome};
...
fn refusal_plain() {
    let e = Event::Refusal {
        source: "cargo".into(),
        reason: "verify budget already spent".into(),
    };
    assert_eq!(
        render(&e, &PLAIN),
        "REFUSED cargo: verify budget already spent"
    );
}
...
fn colour_wraps_in_ansi_and_plain_does_not() {
    ...
    assert!(
        ...
        "coloured render should carry an ANSI escape: {coloured}"
    );
    assert!(
        !plain.contains('\x1b'),
        "plain render must carry no ANSI bytes: {plain:?}"
    );
}
