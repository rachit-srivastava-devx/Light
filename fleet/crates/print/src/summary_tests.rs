...
fn failure_leads_with_next_action_before_the_recap() {
    let mut s = base(Checks::Performed {
        checked: 6,
        total: 8,
    });
    ...
    assert_eq!(
        first_line,
        "next: fix and re-run: fleet gate --id \"semgrep\""
    );
    assert!(
        out.contains("gates    8 attempted -- 6 passed, 2 failed, 0 skipped"),
        "{out}"
    );
}
...
fn success_has_no_next_line_and_publishes_gates_and_checks_separately() {
    ...
    let out = render_summary(&s, &PLAIN);
    assert!(
        !out.starts_with("next:"),
        "a clean run must not fabricate a next action: {out}"
    );
    assert!(
        out.contains("gates    5 attempted -- 5 passed, 0 failed, 0 skipped"),
        "{out}"
    );
...
fn gates_and_checks_are_never_printed_as_one_fraction() {
    ...
    let s = base(Checks::Performed {
        checked: 0,
        total: 0,
    });
    let out = render_summary(&s, &PLAIN);
    assert!(
        !out.contains("checked 0/8"),
        "must not conflate gates-attempted with checks: {out}"
    );
    assert!(
        out.contains("gates    8 attempted -- 6 passed, 2 failed, 0 skipped"),
        "{out}"
...
fn refusals_are_each_a_separate_attributed_line() {
    ...
    let out = render_summary(&s, &PLAIN);
    assert!(
        out.contains("refused cargo: verify budget already spent"),
        "{out}"
    );
}
