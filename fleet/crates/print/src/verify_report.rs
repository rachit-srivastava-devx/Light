...
/// `pub`: `fleet-cli`'s `pipeline::verify_stage` (a separate crate from this one) composes this
/// same event construction for its own per-gate ledger receipts, so `fleet run`'s live gate lines
/// and its durable trail can never drift into disagreeing about what one `GateResult` means (one
/// source of truth, two consumers -- see the pipeline-wiring brief this shares a call site with).
pub fn line_for(r: &GateResult) -> Event {
