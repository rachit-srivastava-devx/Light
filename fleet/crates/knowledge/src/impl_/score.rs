...
use super::item::{ImportanceOutOfRange, MemoryItem, Timestamp};
...
pub fn score(
    ...
    let age_secs = now
        .unix_secs()
        .saturating_sub(item.last_confirmed_at.unix_secs());
    let recency = 0.5_f64.powf(age_secs as f64 / RECENCY_HALF_LIFE_SECS as f64);
    Score(
        weights.alpha * recency
            + weights.beta * item.importance.get()
            + weights.gamma * relevance.get(),
    )
}
