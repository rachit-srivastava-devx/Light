use rollback::{
    rollback, verify_action, ArtifactRecord, GitRollbackPort, Grant, RollbackError,
...
impl Mem {
    fn seed(root: PathBuf, id: &str, s: &str) -> Self {
        let mut m = Self {
            arts: HashMap::new(),
            receipts: vec![],
            root,
        };
        m.arts.insert(id.into(), s.into());
        m
    }
}
impl RollbackStore for Mem {
    fn get_artifact(&self, id: &str) -> Option<ArtifactRecord> {
        self.arts
            .get(id)
            .map(|s| ArtifactRecord { status: s.clone() })
    }
    fn write_receipt(&mut self, r: &RollbackReceipt) {
        self.receipts.push(r.status.clone());
    }
...
impl GitRollbackPort for Git {
    fn remove_owned(&self, _: &Path) -> Result<(), RollbackError> {
        Ok(())
    }
    fn revert(&self, _: &Path, c: &str) -> Result<String, RollbackError> {
        Ok(c.into())
    }
    fn head(&self, _: &Path) -> Result<String, RollbackError> {
        Ok(self.0.clone())
    }
}
...
fn containment_dotdot_inside_root_is_accepted() {
    ...
    let r = rollback(
        &Git("h".into()),
        &mut s,
        req("a1", RollbackTarget::OwnedWorktree(target)),
    );
...
fn verify_removal_detects_path_still_exists() {
    ...
    let r = rollback(
        &Git("h".into()),
        &mut s,
        req("a1", RollbackTarget::OwnedWorktree(wt)),
    );
...
fn verify_action_true_for_ok_correct_status() {
    ...
    assert!(verify_action(&ok, "rolled_back"));
    assert!(!verify_action(&ok, "refused"));
}
#[test]
fn verify_action_false_for_err() {
    let e: Result<RollbackReceipt, RollbackError> =
        Err(RollbackError::ArtifactNotFound("x".into()));
