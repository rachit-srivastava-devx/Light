...
impl MemStore {
    ...
    fn insert(&mut self, id: &str, s: &str) {
        self.artifacts.insert(id.into(), s.into());
    }
}
    ...
    fn remove_owned(&self, path: &Path) -> Result<(), RollbackError> {
        if path.is_dir() {
            std::fs::remove_dir_all(path)
                .map_err(|e| RollbackError::NeedsReconciliation(e.to_string()))
        } else if path.exists() {
...
fn nonexistent_artifact_is_refused() {
    ...
    let result = rollback(
        ...
        req(
            "no-such",
            RollbackTarget::OwnedWorktree(PathBuf::from("/fake/worktrees/wt")),
        ),
    );
...
fn path_escape_is_refused() {
    ...
    let result = rollback(
        ...
        req(
            "art1",
            RollbackTarget::OwnedWorktree(PathBuf::from("../../sensitive")),
        ),
    );
...
fn private_rollback_emits_receipt() {
    ...
    let receipt = rollback(
        &FakeGit,
        &mut store,
        req("art1", RollbackTarget::OwnedWorktree(wt.clone())),
    )
    .unwrap();
