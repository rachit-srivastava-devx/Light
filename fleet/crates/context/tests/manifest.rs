use context::{
    compile, CompileInput, ContextError, ContextManifest, Evidence, RetrievalQuery, Retriever,
    Span, TokenCounter,
};
...
impl Retriever for Fixed {
    fn retrieve(&self, _: &RetrievalQuery) -> Result<Vec<Evidence>, ContextError> {
        Ok(self.items.clone())
    }
...
fn digest_changes_when_evidence_changes() {
    ...
    assert_ne!(
        m1.digest, m2.digest,
        "digest must change when evidence changes"
    );
