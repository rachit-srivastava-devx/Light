...
use review::{
    assemble_reviewed_candidate, CandidateObservation, Decision, Finding, FindingsProvider,
    ReviewError,
};
...
fn same_worker_reviewer_is_not_independent() {
    let e = assemble_reviewed_candidate(
        &obs("d", &[], "same", "same"),
        None,
        Box::new(Fixed(vec![])),
    )
...
fn info_findings_pass_and_decision_is_approved() {
    let r = assemble_reviewed_candidate(
        &obs("d", &["f"], "w", "rev"),
        None,
        Box::new(Fixed(vec![f("INFO")])),
    )
    .unwrap();
...
fn input_digest_is_16_hex_chars() {
    let r = assemble_reviewed_candidate(&obs("d", &[], "w", "r"), None, Box::new(Fixed(vec![])))
        .unwrap();
    assert_eq!(
        r.result.input_digest.len(),
        16,
        "input_digest must be 16 chars"
    );
    assert!(
        r.result.input_digest.chars().all(|c| c.is_ascii_hexdigit()),
        "must be hex"
    );
}
#[test]
fn output_digest_is_16_hex_chars() {
    let r = assemble_reviewed_candidate(&obs("d", &[], "w", "r"), None, Box::new(Fixed(vec![])))
        .unwrap();
    assert_eq!(
        r.result.output_digest.len(),
        16,
        "output_digest must be 16 chars"
    );
    assert!(
        r.result
            .output_digest
