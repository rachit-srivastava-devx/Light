...
impl SeenIds {
    pub fn new() -> Self { Self::with_capacity(MAX_SEEN_IDS) }
    ...
    pub fn record(&mut self, source: &str, delivery_id: &str, payload_digest: &str) -> InboxDecision {
        let key = format!("{source}\x00{delivery_id}");
        let event_id = format!("{source}-{delivery_id}");
        match self.0.get(&key) {
            Some(prior) if prior == payload_digest =>
                InboxDecision::Duplicate { event_id },
            Some(prior) =>
                InboxDecision::Conflict { prior_digest: prior.clone(), new_digest: payload_digest.to_string() },
            None => {
...
impl Default for SeenIds { fn default() -> Self { Self::new() } }
