use offline::{evaluate, EvaluationRequest, HeldOutTask, OfflineError, Outcome, TrialRunner, Variant};
use std::sync::Mutex;
...
struct OrderRecorder { seen: Mutex<Vec<String>> }
...
fn req(ts: Vec<HeldOutTask>, seed: u64) -> EvaluationRequest {
    EvaluationRequest { candidate_id: "c".into(), tasks: ts, min_pairs: 1, seed }
}

fn task(id: &str, digest: &str) -> HeldOutTask {
    HeldOutTask { id: id.into(), input: "x".into(), input_digest: digest.into() }
}
...
fn duplicate_task_same_digest_succeeds() {
    ...
    assert!(result.is_ok(), "identical entries with same digest must succeed, got {result:?}");
}
...
fn trial_order_is_seed_deterministic() {
    ...
    let recorder = OrderRecorder { seen: Mutex::new(Vec::new()) };
    evaluate(&recorder, req(tasks, 0)).unwrap();
    let seen = recorder.seen.lock().unwrap();
    assert_eq!(seen[0], "t0", "with seed=0 and 2 tasks, first trial must be t0");
}
