...
mod tests {
    ...
    fn proposal(worker: &str) -> PlanProposal {
        PlanProposal {
            plan_digest: "pd1".into(),
            worker_id: worker.into(),
            evidence_digest: "ed1".into(),
        }
    }
    ...
    fn proposal_reviewed_by_independent_model() {
        ...
        let events =
            call_independent_model(&proposal("worker-1"), "reviewer-1", &reviewer).unwrap();
        let digest = events.iter().find_map(|e| {
    ...
    fn walkthrough_emitted_before_ready() {
        ...
        let wt_pos = events
            .iter()
            .position(|e| matches!(e, ReviewEvent::Walkthrough(_)));
        let dg_pos = events
            .iter()
            .position(|e| matches!(e, ReviewEvent::Digest(_)));
