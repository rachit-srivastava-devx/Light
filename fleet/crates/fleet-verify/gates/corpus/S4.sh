...
printf '%s\n' 'S4 dispatch receipts were redirected to an unreadable purgeable temp file'
...
for p,ln,s in lines(root):
 if p.suffix in {'.sh','.bash','.zsh'} and re.search(r'(receipt|dispatch)',s,re.I) and re.search(r'mktemp|/tmp',s): hits.append((p,ln,s.strip()))
if hits:
