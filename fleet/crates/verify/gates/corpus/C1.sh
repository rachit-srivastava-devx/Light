...
printf '%s\n' 'C1 a digest-only temp path was concurrently removed'
...
for p,ln,s in lines(root):
 if p.suffix in {'.sh','.bash','.zsh'} and re.search(r'mktemp[^#\n]*(digest|hash|sha|blake|CONTENT)',s.split('#',1)[0],re.I): hits.append((p,ln,s.strip()))
if hits:
