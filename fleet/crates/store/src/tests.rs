use crate::{Event, Record, SqlStore, Store, StoreError, migrate};
...
fn migration_round_trip() {
    ...
    assert_eq!(r1.total, 3, "total must equal the number of known migrations");
...
fn cas_rejects_stale_revision() {
    ...
    let rec = Record { key: key.clone(), revision: 0, payload: b"v1".to_vec() };
...
fn crash_recovery_restores_committed_events() {
    ...
    let _ = std::fs::remove_file(&path);
    {
        ...
        store
            .append_event(0, Event { id: "e1".to_string(), payload: b"hello".to_vec() })
            .unwrap();
    }
