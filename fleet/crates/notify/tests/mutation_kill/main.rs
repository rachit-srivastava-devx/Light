...
fn is_empty_true_when_empty() {
    assert!(Ms::new().is_empty());
}
#[test]
fn is_empty_false_after_enqueue() {
    let mut s = Ms::new();
    enqueue(&mut s, notif("k")).unwrap();
    assert!(!s.is_empty());
}
...
#[test]
fn simple_email_marker() {
    assert_eq!(redact_pii("alice@example.com"), "[REDACTED]");
}
#[test]
fn email_in_sentence_exact() {
    assert_eq!(
        redact_pii("hi alice@example.com done"),
        "hi [REDACTED] done"
    );
}
...
#[test]
fn bare_at_not_redacted() {
    assert_eq!(redact_pii("@example.com"), "@example.com");
}

// redact_pii: special chars in local part must stay in word (kills line 11 == vs != / || vs &&)
#[test]
fn underscore_local_redacted() {
    assert_eq!(redact_pii("a_b@x.com"), "[REDACTED]");
}
#[test]
fn dot_local_redacted() {
    assert_eq!(redact_pii("a.b@x.com"), "[REDACTED]");
}
#[test]
fn hyphen_local_redacted() {
    assert_eq!(redact_pii("a-b@x.com"), "[REDACTED]");
}

// redact_pii: underscore domain start (kills line 13 == vs != on next_is_word check)
#[test]
fn underscore_domain_start_redacted() {
    assert_eq!(redact_pii("u@_d.com"), "[REDACTED]");
}

// redact_pii: special chars inside domain must be consumed (kills line 16 == vs != / || vs &&)
#[test]
fn dash_domain_consumed() {
    assert_eq!(redact_pii("u@a-b.com"), "[REDACTED]");
}
#[test]
fn underscore_domain_consumed() {
    assert_eq!(redact_pii("u@a_b.com"), "[REDACTED]");
}

// redact_pii: kills is_alphanumeric→is_alphabetic on all 3 sites (digit in local/domain leaks PII)
#[test]
fn digit_local_part_redacted() {
    assert_eq!(redact_pii("user1@host.com"), "[REDACTED]");
}
#[test]
fn digit_domain_start_redacted() {
    assert_eq!(redact_pii("user@1host.com"), "[REDACTED]");
}
#[test]
fn digit_domain_mid_redacted() {
    assert_eq!(redact_pii("user@host1.com"), "[REDACTED]");
}
...
fn idem_key_differs_by_task() {
    assert_ne!(
        derive_idempotency_key("t1", "s", "r", "p"),
