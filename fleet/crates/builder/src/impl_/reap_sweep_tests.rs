...
fn sweeps_a_real_dead_worktree_through_the_containment_guarded_remove() {
    ...
    let worktree = merge::create(repo.path(), &name).expect("create");
    super::super::reap::record_worker_pid(&worktree.path, worker);
    ...
    assert!(
        ...
        "the swept worktree must actually be gone"
    );
}
...
fn leaves_a_live_owned_worktree_alone() {
    ...
    let name = merge::unique_name("builder");
    let worktree = merge::create(repo.path(), &name).expect("create");
    super::super::reap::record_worker_pid(&worktree.path, dead_pid());

    assert!(reap_dead_lanes(repo.path()).is_empty());
    assert!(
        worktree.path.exists(),
        "a lane with a live owner must not be touched"
    );
    let _ = merge::remove(repo.path(), &worktree);
}
