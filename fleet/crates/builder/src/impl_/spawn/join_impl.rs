...
pub fn join(
    ...
    super::super::reap::clear_worker_pid(&handle.worktree.path);
    ...
    merge::remove(&handle.repo, &handle.worktree)
        .map_err(|e| JoinError::TeardownFailed(e.to_string()))?;

    Ok((outcome, merge_result?))
}

/// `merge::unique_name` builds `"<label>-<pid>-<seq>"`; the label is the role name this
