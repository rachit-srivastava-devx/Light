...
//! containment-guarded `merge::remove` path `fleet rollback` already uses -- this never
...
use super::reap::find_dead_lanes;
use merge::WorktreeError;
use std::path::{Path, PathBuf};

/// One (path, outcome) pair per lane `find_dead_lanes` proved dead. Never touches a lane it did
/// not prove dead, and never a path outside `<repo>/.worktrees` -- `merge::remove`'s own
