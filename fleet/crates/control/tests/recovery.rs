...
fn ingest_event_drives_state_then_intent() {
    ...
    let t = reduce(
        &snap("t1", TaskState::Pending, 0),
        ev("e1", "t1", TaskState::Pending, TaskState::Running),
        &store,
    )
    .unwrap();
...
fn route_refusal_persists_receipt() {
    let store = MockStore::new();
    let err = reduce(
        &snap("t2", TaskState::Completed, 2),
        ev("e2", "t2", TaskState::Completed, TaskState::Running),
        &store,
    )
    .unwrap_err();
...
fn duplicate_event_idempotent() {
    let store = MockStore::new();
    let t1 = reduce(
        &snap("t3", TaskState::Pending, 0),
        ev("e3", "t3", TaskState::Pending, TaskState::Running),
        &store,
    )
    .unwrap();
