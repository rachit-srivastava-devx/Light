...
fn capability_filter_rejects_missing() {
    ...
    let snapshot = CatalogSnapshot { candidates: vec![candidate], digest: "d1".into() };
    let intent = IntentSpec {
        required_capabilities: vec!["advanced".into()],
        task_class: "x".into(),
    };
    let budget = BudgetSnapshot { limit: 1000, spent: 0 };
...
fn score_boundary_conservative_floor() {
    ...
    let snapshot = CatalogSnapshot { candidates: vec![candidate], digest: "d1".into() };
    let intent = IntentSpec { required_capabilities: vec![], task_class: "x".into() };
    let budget = BudgetSnapshot { limit: u64::MAX, spent: 0 };
...
fn refusal_receipt_contains_first_empty_stage() {
    let snapshot = CatalogSnapshot { candidates: vec![], digest: "d1".into() };
    let intent = IntentSpec {
