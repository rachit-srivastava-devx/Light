...
fn ambiguity_request_produces_research_question() {
    let r = probe(
        &input("OAuth scope"),
        &FakePort("OAuth 2.0 scopes define access".into()),
    )
    .unwrap();
...
fn network_timeout_returns_fallback_question() {
    ...
    assert!(
        body.contains("unable to retrieve")
            || body.contains("timeout")
            || body.contains("unavailable"),
        "expected timeout message in '{}', but not found",
        body
    );
}
...
fn question_references_external_source() {
    let r = probe(
        &input("cert chain format"),
        &FakePort("See the rustls documentation".into()),
    )
    .unwrap();
    let body = r.questions[0].text.to_lowercase();
    assert!(
        ["research", "documentation", "reference", "source"]
            .iter()
            .any(|t| body.contains(t)),
        "expected external source reference in '{}', but none found",
        body
    );
}
...
fn source_fields_are_populated() {
    ...
    assert!(
        (0.0_f32..=1.0).contains(&s.uncertainty),
...
fn empty_query_text_returns_error() {
    let bad = ResearchInput {
        text: "".into(),
        unknown: "x".into(),
        deadline_ms: 100,
    };
    assert!(matches!(
        probe(&bad, &FakePort("t".into())),
        Err(ResearchError::EmptyQuery)
    ));
}
