...
pub fn validate_draft(input: &PlanInput, draft: &PlanDraft) -> Result<ValidationReport, PlannerError> {
    ...
    if draft.modules.len() as u32 > input.max_modules {
        return Err(PlannerError::InvalidDraft(
            format!("module count {} exceeds max {}", draft.modules.len(), input.max_modules)));
    }
    // unique IDs
    let mut seen: HashSet<&str> = HashSet::new();
    for m in &draft.modules {
        if !seen.insert(m.id.as_str()) {
            return Err(PlannerError::InvalidDraft(format!("duplicate id: {}", m.id)));
        }
    }
    ...
    for m in &draft.modules {
        for dep in &m.dependencies {
            if !ids.contains(dep.as_str()) {
                return Err(PlannerError::InvalidDraft(format!("unknown dependency: {dep}")));
            }
        }
    }
    // acceptance refs must be nonempty per module if provided
    if input.acceptance_refs.is_empty() {
        return Err(PlannerError::InvalidInput("acceptance_refs must be nonempty".into()));
    }
...
fn canonical_digest(input: &PlanInput, draft: &PlanDraft) -> String {
    ...
    for m in &draft.modules { m.id.hash(&mut h); }
