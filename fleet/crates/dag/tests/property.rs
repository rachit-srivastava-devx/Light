        ...
        .map(|i| {
            let dep = match seed % 3 {
                0 => {
                    ...
                    if i > 0 { vec![format!("n{}", i - 1)] } else { vec![] }
                }
                1 => {
                    // star: all nodes depend on node 0
                    if i > 0 { vec!["n0".to_string()] } else { vec![] }
                }
                _ => {
                    // binary-tree: node i depends on node i/2
                    if i > 1 { vec![format!("n{}", i / 2)] } else { vec![] }
                }
            };
            make_node(&format!("n{i}"), &dep.iter().map(|s| s.as_str()).collect::<Vec<_>>())
        })
        .collect();
    GraphVersion { id: format!("g{n}_{seed}"), revision: (seed as u64) + 1, nodes }
}
