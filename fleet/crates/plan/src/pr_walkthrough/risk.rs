...
pub(crate) fn riskiest_part(diff: &DiffSummary, acceptance: &[AcceptanceResult]) -> String {
    if let Some(failed) = acceptance.iter().find(|a| !a.passed) {
        return format!(
            ...
            failed.check_name, failed.detail
        );
    }
    let biggest = diff.files.iter().max_by_key(|f| {
        (
            f.lines_added + f.lines_removed,
            std::cmp::Reverse(f.path.clone()),
        )
    });
    match biggest {
        Some(f) => format!(
            "{} has the largest diff ({} lines touched) -- review it first",
            f.path,
            f.lines_added + f.lines_removed
        ),
        None => {
            "no changed files carry a line-count signal; review the diff as a whole".to_string()
        }
    }
}
...
pub(crate) fn reviewer_focus(
    ...
    if !matches!(attestation.verdict, VerdictName::Accept) {
        out.push(format!(
            "attestation verdict is {:?}, not Accept -- read {}'s notes first",
            attestation.verdict, attestation.builder
        ));
    }
