...
fn derive_lesson_from_gate_refusal_is_valid_challenge_input() {
    let outcome = TaughtOutcome::GateRefused {
        check_id: "C1-OPEN",
        detail: "open_questions non-empty".into(),
    };
    let leaf = NodeId::parse("some-leaf").unwrap();
    let lesson = derive_lesson(
        &outcome,
        LessonSource::FailureCorpus("gate-refusal".into()),
        leaf.clone(),
        Role::Builder,
    );
    ...
    let violations =
        validate_challenge_rows(&[row], &atomic_ids, |s| s == "FAILURE-CORPUS:gate-refusal");
...
fn lesson_source_wire_ref_matches_the_two_recognised_prefixes() {
    assert_eq!(
        LessonSource::FailureCorpus("k".into()).as_wire_ref(),
        "FAILURE-CORPUS:k"
    );
    assert_eq!(
        LessonSource::ThreadLessons("k".into()).as_wire_ref(),
        "THREAD-LESSONS:k"
    );
}
...
fn derive_lesson_never_produces_an_empty_mitigation_for_no_killer() {
    let outcome = TaughtOutcome::MutationSurvived {
        mutant: "m1".into(),
        killed_by: None,
    };
    ...
    let lesson = derive_lesson(
        &outcome,
        LessonSource::ThreadLessons("t".into()),
        leaf,
        Role::Verifier,
    );
...
fn derive_lesson_is_pure_and_idempotent() {
    let outcome = TaughtOutcome::Rejected {
    ...
    let a = derive_lesson(
        &outcome,
        LessonSource::FailureCorpus("x".into()),
        leaf.clone(),
        Role::Builder,
    );
    let b = derive_lesson(
