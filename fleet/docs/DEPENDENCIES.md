...
## Rust — `keel/fleet/Cargo.toml` (19 runtime + 2 dev)
...
| `tree-sitter-bash` | 0.23 | bash grammar — the detectors and `bin/` scripts are shell |
...
| `conftest` | Rego policy checks over `policy/` | **UNADOPTED-WITH-REASON** — `arch.rego` does not parse; two pairs pass under legacy-Rego compatibility |
...
## Keyless inference lanes — `bin/lanes.conf`

No API key, no login, no signup on any required path. Ordered failover in `bin/freelane.sh`.
...
rejects and why they failed are in [`LANE-SWEEP.md`](LANE-SWEEP.md).
...
`keel/fleet/Cargo.toml`, `crew/pyproject.toml`, `bin/lanes.conf`, and a grep for invocations across
`bin/*.sh`, `verify.sh` and `tests/corpus/*.sh`. Adoption verdicts are from
[`ADOPTION.md`](ADOPTION.md), which holds the recorded smoke output. The ratio argument — 8,181
hand-written Rust lines against 21 direct crates — is in [`STITCH.md`](STITCH.md).
