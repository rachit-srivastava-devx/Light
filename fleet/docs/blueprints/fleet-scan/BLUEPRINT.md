...
> **As built**, `merge.rs` was split further under the 80-line rule exactly as this section's own
> footnote anticipated (`OpenQuestions`/`OpenQuestionsError`/`Assessment` moved to their own
> `open_questions.rs`, leaving `merge.rs` with just `merge_questions`), and the test layout below is
> organized per-behavior rather than per-source-module — the real `find crates/fleet-scan -name
> '*.rs'` output:
...
crates/fleet-scan/
  ...
  src/
    ...
    input.rs           # RequirementInput, ProbeKind
    fault.rs           # EnvFault
    ports.rs           # CodebasePort, MemoryPort, MemoryHit, ResearchPort, ConcurrentRunner, ProbeJob, ProbeRun
    probe.rs           # Probe trait, ProbeOutcome, Question, GapSeverity
    business.rs        # BusinessProbe (heuristic text checks, no port)
    technical.rs       # TechnicalProbe (uses CodebasePort)
    memory.rs          # MemoryProbe (uses MemoryPort)
    research.rs        # ResearchProbe (uses ResearchPort; the most fault-prone)
    jaccard.rs         # tokenize() + jaccard_similarity()
    merge.rs           # merge_questions()
    open_questions.rs  # OpenQuestions, OpenQuestionsError, Assessment -- split out of merge.rs (per this section's own >80-line contingency note)
    assess.rs          # ProbeSet, AssessmentReport, assess() orchestration
  tests/
    assess_determinism.rs        # assess()/merge_questions() order-independence and repeat-call determinism
    assess_fault_isolation.rs    # one faulted/panicked probe never blocks the other 3 (§9)
    merge_dedup_sort_cap.rs      # drop-no-why / Jaccard dedup / sort-by-gap / cap-4 behavior from §6, §9
    open_questions_rules.rs      # OpenQuestions::new's 0/1..=4/>4 construction rules
    probes_business_technical.rs # BusinessProbe/TechnicalProbe behavior-spec cases
    probes_memory_research.rs    # MemoryProbe/ResearchProbe behavior-spec cases
    common/mod.rs                # shared test fixtures/mock ports
```
