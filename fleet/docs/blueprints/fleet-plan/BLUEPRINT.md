...
- **Post-blueprint addition (as-built, not covered by §3's contract below):** the crate now also
  ships two deterministic, zero-model-call "walkthrough" pipelines — `walkthrough` (`src/walkthrough/`,
  ask #6, "teach the user the implementation plan") and `pr_walkthrough` (`src/pr_walkthrough/`, ask
  #19, "teach the PR it created to the user in detail") — each turning already-validated data (a
  module brief / a diff + acceptance results + attestation) into a structured outline. Both are
  pure, zero-IO, and re-exported from `lib.rs` alongside the five stages this file documents; see
  the real `src/walkthrough/mod.rs` and `src/pr_walkthrough/mod.rs` for their public shape, which
  this blueprint does not itself specify.
...
As built, the crate also owns two further pure, zero-IO walkthrough assemblers not in this
blueprint's original five-stage scope (see §1's "post-blueprint addition" note and §8's file
layout): `walkthrough` (ask #6) and `pr_walkthrough` (ask #19). Both stay inside this crate's
"decisions/assembly over already-in-memory data" charter — neither reads a file or calls a model.
...
> **As built, this tree has grown well past the sketch below** -- several modules were split further
> under the 80-line rule than originally projected (`intake/` gained `atomic_types.rs`, `pattern.rs`,
> `pattern_custom.rs`, `rubric_defs.rs`, `sow_checks.rs`; `ready_gate/` gained `freeze_tail.rs`,
> `gate_checks_c.rs`, `gate_checks_table.rs`, `gate_text_rules.rs`, `module_brief_mid.rs`,
> `module_brief_nested2.rs`), and two whole new modules were added for the post-blueprint
> walkthrough work (see §1/§2's "post-blueprint addition" note): `pr_walkthrough/` and
> `walkthrough/`. The list below is the real `find crates/fleet-plan -name '*.rs'` output, not the
> original projection.
...
crates/fleet-plan/
  ...
  src/
    ...
    intake/
      mod.rs                     # re-exports for the intake module
      question_id.rs             # QuestionId, UnknownQuestionId, parse/as_str
      rubric.rs                  # IntakeQuestion, Trigger, derive_questions, open_questions
      rubric_defs.rs             # the 13 derived-question trigger definitions, split out of rubric.rs
      pattern.rs                 # trigger-matching primitives derive_questions relies on
      pattern_custom.rs          # per-id custom trigger patterns, split out of pattern.rs
      sow.rs                     # StageViolation, validate_sow_text
      sow_checks.rs              # validate_sow_text's individual field checks, split out of sow.rs
      atomic.rs                  # AtomicTier, AtomicRow, validate_atomic_rows
      atomic_types.rs            # AtomicRow/AtomicTier's supporting types, split out of atomic.rs
      challenges.rs              # ChallengeRow, validate_challenge_rows
      clarifications.rs          # ClarificationKind, ClarificationRow, validate_clarification_rows
      gate.rs                    # StageReadiness, GateDecision, intake_gate
    lld_draft.rs                 # assemble_blueprint_doc, assemble_acceptance_checks_draft, assemble_summary_doc
    ready_gate/
      mod.rs                     # re-exports for the ready_gate module
      violation.rs               # Violation
      module_brief_core.rs       # is_non_empty_str/str_trim_len/as_str/is_string_array + node_id/freeze_id/content_hash validators (lld.rs:37-127)
      module_brief_fields.rs     # ALLOWED_*/FORBIDDEN_* const arrays (lld.rs:129-178)
      module_brief.rs            # validate_module_brief part 1: top-level scalar fields (lld.rs:182-275)
      module_brief_mid.rs        # validate_module_brief part 1.5, split out of module_brief.rs
      module_brief_nested.rs     # validate_module_brief part 2: registry/acceptance/guarantees/alternatives/failure_story (lld.rs:275-425)
      module_brief_nested2.rs    # validate_module_brief part 2.5, split out of module_brief_nested.rs
      registry_verdict.rs        # check_registry_verdict (lld.rs:428-456)
      accepts_when.rs            # check_accepts_when (lld.rs:459-519)
      freeze.rs                  # check_freeze part 1: scalar fields (lld.rs:521-587)
      freeze_depth_evidence.rs   # check_freeze part 2: depth_evidence/supersedes/stamped_by (lld.rs:588-667)
      freeze_tail.rs             # check_freeze part 3, split out of freeze_depth_evidence.rs
      sow_seed.rs                # check_sow_seed (lld.rs:669-699)
      lld_v1.rs                  # validate_lld_v1 (lld.rs:701-755)
      canonical.rs                # NumericLeafError, canonical_json, hex_encode, hash_canonical_string, content_hash (lld.rs:757-827)
      gate_types.rs                # GateRefs, GateReason, DepthScore, Outcome, Verdict, Check, GATE_CHECK_IDS, STAMPED_BY
      gate_eval.rs                  # evaluate, evaluate_with, round3, depth_evidence, EntryOutcome, check_and_evaluate
      gate_checks_a.rs               # 1st third of the 14 depth checks (lld_ready.rs:317-431)
      gate_checks_b.rs                # 2nd third of the 14 depth checks (lld_ready.rs:433-511)
      gate_checks_c.rs                 # 3rd third of the 14 depth checks, split out of gate_checks_b.rs
      gate_checks_table.rs              # the CHECKS: &[Check; 14] table, split out of gate_eval.rs
      gate_text.rs                       # as_str/trim_len/as_array/as_str_array/is_word_char/contains_word_ci/contains_absolute_claim/matches_tautology/valid_revive_trigger (lld_ready.rs:212-311)
      gate_text_rules.rs                  # gate_text.rs's rule-matching helpers, split out of gate_text.rs
    review/
      mod.rs                       # re-exports for the review module
      contract.rs                  # RoleContract, ReviewContractViolation, validate_review_contract
      submission.rs                # SubmissionRefusal, submission_eligible
      verdict.rs                   # VerdictName, VerdictDecision, VerdictRefusal, verdict_decision
    teach.rs                       # LessonSource, TaughtOutcome, Lesson, derive_lesson
    pr_walkthrough/                # NEW (post-blueprint, see §1/§2): ask #19 "teach the PR to the user"
      mod.rs                       # re-exports
      build.rs                     # build_pr_walkthrough
      error.rs                     # PrWalkthroughError
      risk.rs                      # risk-summary assembly
      types.rs                     # AcceptanceResult, AttestationSummary, DiffSummary, FileChange, PrWalkthrough, VerifiedItem
    walkthrough/                   # NEW (post-blueprint, see §1/§2): ask #6 "teach the plan to the user"
      mod.rs                       # re-exports
      build.rs                     # build_walkthrough
      error.rs                     # WalkthroughError
      extract.rs                   # pub(crate) extraction helpers over already-validated data
      sections.rs                  # section assembly
      types.rs                     # AcceptancePreviewItem, BuildItem, DecisionItem, RiskItem, Walkthrough, WorkOrderItem
      work_order.rs                # WorkOrderItem assembly
  tests/
    intake_rubric.rs                    # derive_questions/open_questions/intake_gate behavior-spec cases
    intake_challenges_clarifications.rs # validate_challenge_rows/validate_clarification_rows
    intake_sow_atomic.rs                # validate_sow_text/validate_atomic_rows
    lld_draft.rs                        # assemble_* text-assembly cases
    ready_gate_shape.rs                 # validate_module_brief/validate_lld_v1/canonical_json/content_hash (ported from lld.rs's own #[cfg(test)] module)
    ready_gate_depth.rs                 # evaluate/evaluate_with/depth_evidence/check_and_evaluate (ported from lld_ready.rs's own #[cfg(test)] module)
    review_decision.rs                  # validate_review_contract/submission_eligible/verdict_decision
    teach.rs                            # derive_lesson behavior-spec cases
    pr_walkthrough.rs                   # build_pr_walkthrough behavior-spec cases
    pr_walkthrough_risk.rs              # risk-summary assembly cases
    walkthrough.rs                      # build_walkthrough behavior-spec cases
    walkthrough_sections.rs             # section-assembly cases
```
