...
| `tokio-stream` | `0.1`, `features = ["sync"]` (post-build addition — **not** in the brief's dependency list) | `DashboardSink`'s SSE route needs an `impl Stream` to bridge a `tokio::sync::broadcast::Receiver<StreamEvent>` into axum's `Sse<S>` response type; `tokio-stream`'s `wrappers::BroadcastStream` is the minimal glue for that. Flagged in the crate's own `Cargo.toml` comment as a deviation from the brief — see the divergence note at the end of this file. |

`OtelSink` uses `std::process::Command` (already in `std`, no new dep); `FileSink` uses `std::fs`.
The brief named three new crates (`axum`, `tokio-tungstenite`, `reqwest`); the built crate needed a
fourth (`tokio-stream`, above) beyond what this section originally said "no other crate is needed"
about.
...
crates/fleet-stream/
  ...
  src/
    ...
    event.rs             # ~16 — StreamEvent
    log_source.rs         # ~27 — LogSource, LogSourceError
    cursor.rs              # ~19 — CursorStore, CursorError
    sink.rs                 # ~30 — Sink, SinkError
    pump/
      mod.rs                # ~12 — re-exports
      config.rs             # ~49 — PumpConfig, RetryBackoff
      stats.rs               # ~27 — SinkStats
      worker.rs               # ~50 — run_sink: per-cycle poll/cursor-load, delegates a cycle's work to worker_cycle
      worker_cycle.rs          # ~58 — one poll cycle's filter/deliver/retry/persist body, split out of worker.rs to hold the 80-line rule (not anticipated by this section)
      orchestrate.rs           # ~37 — pump: spawns one worker task per (sink, config)
    sinks/
      mod.rs                 # ~20 — re-exports
      orb.rs                  # ~57 — OrbSink: lane_status filter + POST
      orb_project.rs           # ~41 — the lane-status.v1.json projection itself, split out of orb.rs (not anticipated by this section)
      dashboard/
        mod.rs                 # ~55 — DashboardSink struct + Sink impl (delegates to server/broadcast)
        server.rs               # ~59 — lazy axum app bring-up on 127.0.0.1:0, SSE + WS routes
        broadcast.rs             # ~28 — the broadcast::Sender<StreamEvent> + subscribe helper
      meter.rs                  # ~68 — MeterSink: est-tokens/window%/labelled-$ aggregation
      meter_types.rs             # ~16 — MeterSample/MeterSnapshot types, split out of meter.rs (not anticipated by this section)
      webhook.rs                  # ~64 — WebhookSink: config-only allow-list + reqwest POST
      otel.rs                      # ~63 — OtelSink: subprocess to telemetry_otel.py span
      otel_payload.rs               # ~25 — the JSON-on-stdin payload shape, split out of otel.rs (not anticipated by this section)
      file.rs                       # ~67 — FileSink: append-only NDJSON writer
  tests/
    log_source_fake.rs            # ~68 — shared in-memory LogSource/CursorStore test fakes
    sink_fake.rs                    # ~72 — shared configurable-failure test-double Sink (not anticipated by this section)
    run_sink_ordering.rs              # ~54 — ordering/backlog/cursor-advance behavior (§9)
    run_sink_resume.rs                  # ~58 — resuming from a persisted cursor after a simulated crash (not anticipated by this section)
    run_sink_retry.rs                     # ~73 — Transient retry-then-Permanent-downgrade behavior
    two_sinks_independent.rs                # ~74 — a blocked sink never backpressures a healthy sink (not anticipated by this section)
    sink_accepts_filtering.rs                 # ~72 — accepts() per sink type, table-driven
    file_sink_ndjson.rs                         # ~33 — FileSink writes valid NDJSON, one line per event
    meter_sink_honesty.rs                         # ~25 — MeterSink never fabricates a $ figure (not anticipated by this section)
    orb_sink_projection.rs                          # ~79 — OrbSink's body matches lane-status.v1.json shape
```
> The originally-sketched `worker.rs`, `sinks/orb.rs`, `sinks/meter.rs`, and `sinks/otel.rs` were
> each split further once bodies landed (`worker_cycle.rs`, `orb_project.rs`, `meter_types.rs`,
> `otel_payload.rs`), as this section anticipated in general terms. Verify with
...
# Not in the brief's dependency list; added because axum's Sse<S> needs a Stream to bridge a
# tokio::sync::broadcast::Receiver, and this is the minimal glue crate for that -- flagged as a
# deviation in the crate's return summary.
...
1. **`fleet-store` has no blueprint yet, so `LogSource`/`CursorStore` are this crate's proposal for
   ...
   `../fleet-store/BLUEPRINT.md` to check this crate's `LogSource` trait against. This
