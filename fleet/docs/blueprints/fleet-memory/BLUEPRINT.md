...
- **Imported by:** `fleet-govern` (consumes `pick_arm` for adaptive model selection as a signal
  ...
  > directly. Neither crate has a written blueprint yet (`../fleet-store/` and
  > `../fleet-context/` are empty as of this writing). This blueprint therefore does NOT take
...
crates/fleet-memory/
  Cargo.toml
  src/
    lib.rs            # ~27 — module decls + re-exports only
    ident.rs           # ~42 — MemoryId, EmptyMemoryId, MemoryKind
    embedding.rs       # ~58 — Embedding, CosineSimilarity, EmptyEmbedding, DimensionMismatch, cosine()
    item.rs            # ~54 — Timestamp, Importance, ImportanceOutOfRange, MemoryItem
    score.rs           # ~46 — Relevance, ScoreWeights, Score, RECENCY_HALF_LIFE_SECS, score()
    retrieve.rs        # ~77 — LexicalHit/VectorHit/LexicalSearch/VectorSearch/RetrieveError/RetrievedItem, retrieve()
    fusion.rs          # ~22 — RRF_K, fuse_rrf() — split out of retrieve.rs to keep it under the line budget, as this section anticipated
    dedup.rs           # ~78 — NewMemory, NearestNeighborLookup, DedupThreshold, WriteDecision, write()
    bandit/
      mod.rs           # ~9 — module decls + re-exports
      arm.rs           # ~15 — ArmStats, NoArms
      random.rs        # ~37 — RandomSource trait, gamma_sample(), normal_sample() (Marsaglia-Tsang + Box-Muller)
      thompson.rs      # ~30 — beta_sample() (ratio of two gamma_sample calls), pick_arm()
    promote.rs         # ~75 — DiffPattern, PromotionScope, PromotedLesson, PromotionRefusal, promote_lesson()
    gate_check.rs      # ~34 — PatternMatcher, PatternError, check_added_line() — split out of promote.rs (not anticipated by this section; see the divergence note on the `Box::leak` this file uses to bridge a dynamic `lesson.category` into `GateRefusal`'s `&'static str` code field)
  tests/
    score_recency.rs        # ~60 — score() edge cases (§9)
    retrieve_fusion.rs       # ~56 — RRF fusion + fail-fast port errors (§9)
    retrieve_determinism.rs # ~34 — retrieve() tie-breaking/idempotence cases (not anticipated by this section)
    dedup_threshold.rs       # ~66 — write() Insert/Merge boundary cases (§9)
    bandit_thompson.rs       # ~61 — pick_arm() determinism + exploration cases (§9)
    promote_and_check.rs     # ~57 — promote_lesson()/check_added_line() cases (§9)
    support/mod.rs           # ~35 — shared test-double LexicalSearch/VectorSearch/NearestNeighborLookup/PatternMatcher/RandomSource implementations (not anticipated by this section)
```
> The originally-sketched single `retrieve.rs` and `promote.rs` were each split further once bodies
> landed (`fusion.rs` out of `retrieve.rs`; `gate_check.rs` out of `promote.rs`), as this section
> anticipated. The file-size gate (§10) runs before Opus review.
...
1. **No compile dependency on `fleet-store`/`fleet-context`.** MIGRATION-PLAN's DAG
   ...
   blueprint yet (both `../fleet-store/` and `../fleet-context/` are empty). This
...
5. **(post-build, suspected code bug — NOT fixed by this pass) `write`'s `Merge` outcome does not
   honor this blueprint's documented contract for `new_importance`/`new_confirmed_count`.** §3.F's
   `WriteDecision::Merge` doc comment says the merge fields should be "the max of the two" existing
   vs. candidate importance (never lower it) and `confirmed_count` incremented from the existing
   item's real count. The built `write` (`crates/fleet-memory/src/dedup.rs:41-67`) cannot honor
   this: `NearestNeighborLookup::nearest` (dedup.rs:17-19) returns only `(MemoryId,
   CosineSimilarity)`, never the existing item's `importance`/`confirmed_count`, so the shipped
   code sets `new_importance: candidate.importance` (the candidate's own value, not a max) and
   `new_confirmed_count: 1` unconditionally (dedup.rs:63-64) — not `existing.confirmed_count + 1`.
   The code's own doc comment (dedup.rs:44-50) acknowledges this as a "documented deviation" and
   pushes the real max/increment logic onto the caller, which must look up the existing item
   itself and combine it with these fields. No test in `tests/dedup_threshold.rs` exercises
   `new_confirmed_count`/`new_importance`'s value at all, so nothing currently catches a caller
   that trusts these fields' doc comment literally and skips its own max/increment step — flagged
   here rather than silently rewritten into either the blueprint or the code, since it is unclear
   whether the fix belongs in `write`'s contract (change `NearestNeighborLookup` to return the full
   item) or purely in caller-side documentation.
6. **(post-build, worth a second look — NOT fixed by this pass) `check_added_line` bridges a
   dynamic `String` into `GateRefusal`'s `&'static str` code field via `Box::leak`.**
   `fleet-types::GateRefusal::new` (`crates/fleet-types/src/gate_refusal.rs:15`) requires `code:
   &'static str` by design (a roster-wide, compile-time-constant refusal-code shape). But
   `PromotedLesson.category` (this crate) is a caller-supplied, runtime `String` (from
   `promote_lesson`'s `category: &str` parameter), not a compile-time constant. The shipped
   `check_added_line` (`crates/fleet-memory/src/gate_check.rs:23`) resolves the mismatch with
   `Box::leak(lesson.category.clone().into_boxed_str())` — a deliberate, permanent per-distinct-
   category memory leak, called out in the file's own doc comment as "flagged for Opus." This
   blueprint's original §3 signature for `check_added_line` did not anticipate this: it just says
   the returned `GateRefusal`'s `code` "is `lesson.category`," with no mention of the `&'static
   str` requirement or a leak. Not fixed here since resolving it is a design decision (e.g.
   loosening `GateRefusal::code` to `Cow<'static, str>` or `String`, which is a `fleet-types`
   change out of this crate's/this pass's scope) rather than a doc-staleness fix.
