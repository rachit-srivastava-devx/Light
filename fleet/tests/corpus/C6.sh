...
printf '%s\n' 'C6 generator and verifier identity was required without a no-model sentinel'
...
for p,ln,s in lines(root):
 if re.search(r'\b(gen|generator)\s*(!=|==)\s*(ver|verifier)',s,re.I): hits.append((p,ln,s.strip()))
if hits:
