...
fn assert_identity_fields(v: &serde_json::Value) {
    ...
    assert!(
        matches!(tree_state.as_str(), "clean" | "dirty" | "unknown"),
        "got {tree_state:?}"
    );
    let build_time = get(v, "build_time");
    assert!(
        build_time.contains('T') && build_time.ends_with('Z'),
        "not RFC3339 UTC: {build_time:?}"
    );
}
...
fn version_human_output_mentions_sha_and_build_time() {
    ...
    assert!(
        ...
        "human `version` missing commit_sha: {text}"
    );
    assert!(
        text.contains("build_time"),
        "human `version` missing build_time: {text}"
    );
}
