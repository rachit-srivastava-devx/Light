...
fn run_json_carries_per_stage_outcomes_gate_verdicts_and_the_classification() {
    ...
    assert!(
        v["classification"].is_object(),
        "classify PASSed, so it must be reported: {v}"
    );
    ...
    for s in &stages {
        assert_eq!(
            s["outcome"], "pass",
            "stage {} did not pass: {s}",
            s["stage"]
        );
        assert!(
            s["elapsed_ms"].is_u64(),
            "a stage that ran must publish a duration: {s}"
        );
    }
    ...
    assert_eq!(
        ids.len(),
        8,
        "every registry gate must be reported, got {ids:?}"
    );
    assert!(ids.contains(&"unit tests"), "got {ids:?}");
    let unit = gates.iter().find(|g| g["id"] == "unit tests").unwrap();
    assert_eq!(
        unit["verdict"], "pass",
        "the configured npm-style command must run: {unit}"
    );
    assert_eq!(
        unit["checked"], 3,
        "the gate's published denominator must survive: {unit}"
    );
...
fn a_refused_run_names_the_failing_gate_and_the_refusal_in_json() {
    ...
    let refusal = v["refusal"]
        .as_str()
        .expect("a refused run must publish its reason");
    assert!(
        refusal.contains("detectors"),
        "refusal must name the gate: {refusal}"
    );
