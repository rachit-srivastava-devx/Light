...
/// `dispatch::verify_repo::ensure_repo` (shared by `gate`/`oracle`/`run`/`swarm`, see PR #15)
/// now requires `--repo` to be a real git worktree, not just a readable directory -- a fresh
/// `tempdir()` alone is refused before the gate ever runs. `git init` is enough: the check is
/// `git rev-parse --show-toplevel`, which succeeds with zero commits.
...
fn a_scratch_repos_failing_test_is_reported_without_compiling_fleets_own_tree() {
    ...
    assert!(
        !out.status.success(),
        "a real failing test in --repo must fail the gate: {stderr}"
    );
    ...
    assert!(
        stderr.contains("0/1"),
        "must report the SCRATCH crate's own 1-test denominator: {stderr}"
    );
    assert!(
        !stderr.contains("fleet-types"),
        "must not compile fleet's own workspace: {stderr}"
    );
    assert!(
        !stderr.contains("fleet-cli"),
        "must not compile fleet's own workspace: {stderr}"
    );
...
fn the_same_scratch_repo_with_a_passing_test_reports_pass() {
    ...
    assert!(
        out.status.success(),
        "a real passing test in --repo must pass the gate: {stderr}"
    );
    assert!(
        stderr.contains("1/1"),
        "must report the SCRATCH crate's own 1-test denominator: {stderr}"
...
fn an_unusable_repo_is_a_typed_refusal_never_a_verdict_about_a_different_directory() {
    ...
    let out = cmd()
        .args(["gate", "--repo", bogus])
        .output()
        .expect("binary runs");
    assert!(
        !out.status.success(),
        "a nonexistent --repo must never exit 0"
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains(bogus),
        "the typed error must name the bad repo: {stderr}"
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.is_empty(),
        "no verdict should print for a refused target: {stdout}"
    );
}
