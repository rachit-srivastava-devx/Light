...
//! delegating to `fleet_merge::pr_emit`. Split out of `ops_cmd.rs` to keep it under the 80-line
...
pub fn pr_emit(args: PrArgs) -> Result<(), DispatchError> {
    ...
    let outcome = fleet_merge::pr_emit(repo, repo, &args.branch, &args.module_brief, &args.diff_summary)?;
