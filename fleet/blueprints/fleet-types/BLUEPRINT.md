...
/// `receipt.v1.json`'s `event` enum (`receipt.v1.json:15`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
...
crates/fleet-types/
  Cargo.toml
  src/
    lib.rs        # re-exports; module wiring only
    role.rs        # Role, UnknownRole
    ident.rs       # TaskId, NodeId, LaneId, EmptyIdentifier, BadNodeId
    tokens.rs      # Tokens, TokensOverflow
    lifecycle.rs   # LifecycleState, GateRefusal
    exit_code.rs   # ExitCode, UnknownExitCode
    receipt.rs     # ~55 — Blake3Hash, PrevHash, BadBlake3Hash (the hash newtypes)
    receipt_event.rs # ~55 — ReceiptEvent, SchemaV1, Receipt (the wire record)
    attest_subject.rs  # ~55 — DeliveryTier, BareBlake3Digest, AttestationSubject, AttestationDigest
    attest_predicate.rs# ~55 — DeliveryPredicate, AttestationElements, DeliveryAttestationV1
    attest_stmt.rs     # ~55 — InTotoStatementV1, Attestation, AttestationBuilder
  tests/
    receipt_roundtrip.rs      # serde round-trip + edge cases against receipt.v1.json
    attestation_roundtrip.rs  # serde round-trip + edge cases against attestation.v1.json
    identifiers.rs            # TaskId/NodeId/LaneId parse edge cases
```
...
- `receipt_roundtrip.rs::receipt_matches_the_json_schema_shape` — construct a `Receipt` for each
  of the 7 `ReceiptEvent` variants, serialize, and assert against a literal expected-JSON string
