...
| Package | Version | Why |
|---|---|---|
| `pydantic` | `2.13.4` (matches `fleet/crew/uv.lock`'s already-resolved version; `pyproject.toml` pins `>=2.0,<3.0`) | `schema/*.py`'s strict, `extra="forbid"` mirrors of the three JSON contracts — this package's only validation framework. |
| `numpy` | `2.4.6` (matches `uv.lock`; `pyproject.toml` pins `>=2.0,<3.0`) | `parity/analyze.py`'s array math (`np.asarray`, `np.mean`, `np.var`). |
| `statsmodels` | `0.14.6` (matches `uv.lock`; `pyproject.toml` pins `>=0.14,<1.0`) | `CompareMeans`/`DescrStatsW`/`ttost_ind` — the Welch's-TOST implementation itself; reimplementing this by hand would be exactly the kind of "adopting a tool is not the tool working" risk this repo's own `PRINCIPLES.md` warns against — use the maintained library. |
| `pytest` (dev) | `8.4.2` (matches `uv.lock`; `pyproject.toml` pins `>=8.0,<9.0`) | The existing test suite's runner. |

No new dependency is introduced by this blueprint — every package above is already resolved in
`fleet/crew/uv.lock` today; the move to `crates/fleet-crew/` changes paths, not the dependency set.
...
crates/fleet-crew/
  pyproject.toml
  crew/
    __init__.py                # ~5  — package marker only
    adapters/
      __init__.py              # ~20 — re-exports (Adapter, AdapterError, ClaudeAdapter, CodexAdapter, ...)
      protocol.py               # ~40 — Adapter Protocol
      errors.py                  # ~10 — AdapterError
      records.py                  # ~35 — UsageRecord, InvocationResult
      metadata.py                  # ~45 — read_metadata
      runner.py                     # ~75 — run_model (subprocess exec + floor/diff invariants)
      _cli_adapter.py                # ~45 — invoke_cli(): the shared body claude.py/codex.py both call
      claude.py                       # ~25 — ClaudeAdapter (command() + thin invoke())
      codex.py                         # ~25 — CodexAdapter (command() + thin invoke())
      capability_report.py              # ~50 — CapabilityReport dataclass + 4 properties
      capability_probe.py                # ~45 — probe_adapter()
    sow/
      __init__.py                        # ~15 — re-exports (build_sow, validate_sow, Sow, ...)
      errors.py                           # ~20 — SOWRefusal
      model.py                             # ~65 — MachinePredicate, AtomicLeaf, Challenge, Clarification, Alternative, Sow
      _paths.py                             # ~10 — _default_repo_root
      parse_sections.py                      # ~55 — _clean_task, _split_sections, _strip_bullet
      parse_leaves.py                         # ~40 — _build_leaves
      parse_meta.py                             # ~75 — _find_citations, _parse_challenges, _parse_clarifications
      parse_meta_ext.py                          # ~35 — _parse_alternatives, _parse_estimate, _parse_edge_cases, _is_generic_question
      citations.py                                # ~40 — _resolve_file, _validate_citation
      validate.py                                  # ~40 — validate_sow
      build.py                                      # ~65 — build_sow
      payload.py                                     # ~50 — _sow_payload
      cli.py                                          # ~70 — main() + argparse
    schema/
      __init__.py                                     # ~15 — re-exports + DeliveryAttestation/InTotoStatement aliases
      contract_model.py                                # ~10 — ContractModel
      attestation.py                                     # ~75 — Digest, Subject, AttestationElements, AttestationBuilder, AttestationPredicate, Attestation
      receipt.py                                          # ~30 — Receipt (+ ts_wall validator)
      submission.py                                        # ~10 — Submission
    parity/
      __init__.py                                          # ~15 — re-exports
      types.py                                              # ~55 — ALPHA/MIN_TOTAL_N/MIN_PER_PROMPTER, ParityRefusal, Observation
      result.py                                              # ~45 — ParityResult (+ as_dict)
      read.py                                                # ~50 — _finite_number, read_observations
      analyze.py                                             # ~80 — analyze_parity (see §5's note — the one fn that lands exactly at the cap and cannot split further without breaking one statistical derivation mid-flow; flagged, not silently exempted)
      cli.py                                                 # ~50 — _open_input, main, __all__, entry point
  tests/
    test_adapter_claude.py         # ~50 — ClaudeAdapter invoke/resolved-model/log-floor/diff cases
    test_adapter_codex.py          # ~20 — CodexAdapter command-shape parity with claude (thin, most logic shared/tested once)
    test_capability_probe.py       # ~45 — probe_adapter's builder/verifier eligibility cases
    test_sow_build.py              # ~60 — build_sow happy path + structural refusals
    test_sow_validate.py           # ~45 — validate_sow citation/generic-question refusals
    test_sow_cli.py                # ~40 — python -m crew.sow.cli end-to-end
    test_schema_roundtrip.py       # ~50 — Receipt/Attestation/Submission parse + extra="forbid" rejection
    test_parity_analyze.py         # ~70 — analyze_parity equivalence/underpowered/refusal cases
    test_parity_read.py            # ~30 — read_observations malformed-line cases
```
> Any file still over 80 lines once real bodies land splits further (e.g. `parse_meta.py` →
> `parse_challenges.py` + `parse_clarifications.py`) — the §10 `wc -l` gate is the actual check,
> these counts are estimates from the real line ranges cited in §5, not a promise.

`pyproject.toml` (unchanged from today, path/name aside):
```toml
[project]
name = "crew"
version = "0.1.0"
...
3. **`parity/analyze.py` is the one file in this blueprint's §8 layout that lands right at the
   80-line ceiling and may not fit** — `analyze_parity`'s ~105-line body (today's `parity.py:143-
   247`) is one continuous statistical derivation (prompter-count check → per-group variance check
   → `ttost_ind` → CI → joint verdict logic); splitting it mid-derivation into two functions would
   force passing 5+ intermediate values across the split and make the derivation harder to audit
   against the TOST literature, not easier. This is the file this brief asked to be named explicitly
   if it can't get under 80 without an awkward split — **flagging it now**: recommend Opus either
   accept one named exception file with a comment explaining why, or accept the awkward split
   (`analyze_core.py` computing statistics + `analyze_verdict.py` computing the verdict from them).
4. **Adding `ruff` as a dev dependency** (§10) — not in today's `pyproject.toml`. The build brief
   ...
   addition.
5. **Two small, real gaps surfaced while writing §6** (not failures of today's code so much as
   untested edges): a negative `log_size_floor` would trivially pass the floor check, and a
   non-UTF-8 transcript byte would raise `UnicodeDecodeError` out of `read_metadata` instead of the
   documented "absence of metadata is not an error" contract. Both get named tests in §9 and a
   one-line fix each in §5/§6 — small enough to fix in the same PR as the extraction, not deferred.
