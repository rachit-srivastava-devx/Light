...
| negative | n/a. |
| duplicate | Calling `remove` twice on the same already-removed `Worktree`: `git worktree remove --force` fails (nothing to remove) but `worktree.path.exists()` is already `false` → treated as success, `Ok(())` (mirrors `worktree.rs:134-141`'s "already gone is not a leak" rule) — idempotent by design. |
| concurrent | Two callers racing `remove` on the *same* `Worktree` value: the second caller's `git worktree remove` fails but finds the path already gone → `Ok(())` per the idempotence rule above; no double-free-style error. |
...
crates/fleet-merge/
  Cargo.toml
  src/
    lib.rs         # ~12 — module decls + re-exports only
    error.rs       # ~55 — WorktreeError, MergeRefusal, both exit_code() impls
    git_exec.rs    # ~45 — run_git(repo, args) -> Result<Output, Spawn-mapped error>; shared subprocess helper used by worktree.rs and merge.rs, avoiding duplicated Command::new("git") boilerplate
    worktree.rs    # ~75 — Worktree, unique_name, create, remove (ports worktree.rs:27-164, minus lane_cap)
    invariant.rs   # ~40 — check_stage_nonempty, check_head_moved, check_files_changed (pure, no git_exec import)
    merge.rs       # ~70 — MergeOutcome, merge_lane (calls git_exec + invariant)
    cache.rs       # ~20 — invalidate_build_cache
  tests/
    worktree_lifecycle.rs   # ~65 — create/remove round trip, invalid-name refusal, idempotent double-remove, name-collision-free, all against a real tempdir repo
    merge_invariants.rs     # ~85 — happy-path merge_lane end-to-end + all three refusal paths (empty-stage, head-unmoved via already-merged branch, conflict), against real tempdir repos
    invariant_pure.rs       # ~30 — check_stage_nonempty/check_head_moved/check_files_changed unit tests with no git process at all
```
> If `merge_invariants.rs` still projects over 80 lines once bodies land, split into
> `merge_happy.rs` / `merge_refusals.rs`. The file-size gate (§10) is run before Opus review.
...
- `create_twice_with_same_name_fails_on_retry_exhaustion` — the "duplicate" case from §6: second
  ...
  tuning.
- `remove_is_idempotent_on_an_already_removed_worktree` — `remove` called twice on the same
  `Worktree` value both return `Ok(())`.
