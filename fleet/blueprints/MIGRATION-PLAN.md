# Fleet — Migration & Build Plan (living doc)

> **Resume anchor.** If the Opus session dies (quota), any agent — a fresh Claude, or `opencode` —
> reads THIS file first and continues. Every crate's state is in the table below. Never trust a
> checkbox without re-running the crate's `verify` (see §6). Update this file after every crate
> state change.

Last updated: 2026-09-08 · Owner: rachit · Lead/review: Opus · Build: Sonnet + opencode

> **PROJECT STATUS: 16/16 crates DONE** (all green: tests + clippy -D warnings + ≤80/file; workspace
> `cargo test` exit 0). Single-HTML report at `blueprints/fleet.html` (part 5) ✓.
> **Remaining:** (a) a git checkpoint commit — nothing under `crates/`/`src/` is tracked yet (awaiting
> owner go-ahead); (b) two open human-judgment items — the worker `probe_no_ambient` keyless proof and
> the plan lifecycle-state mapping; (c) P6 cleanup DEFERRED until those close (deleting the source
> `fleet/keel` before the new system is proven end-to-end would be premature — the goal says clean
> "once all migration AND OTHER THINGS are done").

**Every agent follows `blueprints/PLAYBOOK.md`** (AI-native SDLC: spec-before-code, verify-own-work
with a published denominator, no self-grading, mistakes→teach-back). The blueprint template is
`blueprints/_TEMPLATE.md`; the worked exemplar is `blueprints/fleet-router/BLUEPRINT.md`.
...
Light/fleet/          # <- the new workspace lives INSIDE fleet/ (moved here 2026-09-08 per owner)
  ...
  blueprints/         # one folder per crate; each holds BLUEPRINT.md (hand-codeable / 4B-completable)
    _TEMPLATE.md      # the house blueprint template (Sonnet fills per crate; Opus reviews)
    _BUILD-BRIEF.md   # how a build agent implements a crate (trust the blueprint, no cargo mutants)
...
`blueprints/<crate>/BLUEPRINT.md`. 4. Continue at its `state`. 5. `opencode` can build a crate solo
