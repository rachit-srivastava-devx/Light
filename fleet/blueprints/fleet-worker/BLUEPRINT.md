...
| `libc` | `0.2` (pin to `0.2.189`, already resolved in `fleet/keel/Cargo.lock`) | `socketpair`, `dup2`, `setsid`, `recv`, `send`, `kill` — the exact fd-3 primitives `main.rs` already uses; no reason to introduce a higher-level wrapper for syscalls this codebase has already hand-verified (the Darwin `SOCK_SEQPACKET`→`SOCK_STREAM` fallback is a real, tested platform difference, not decoration). |
| `nix` | `0.29` (not yet in fleet's lock — greenfield addition per task brief) | Safer, typed wrappers for `RLIMIT_AS` (`nix::sys::resource::setrlimit`) and `setpgid` (`nix::unistd::setpgid`) supervision this crate adds beyond what `main.rs` does today (today only calls `setsid`, not an explicit `RLIMIT_AS`) — chosen over hand-rolled `libc` calls for these two because `nix` is a long-maintained, widely-used (5000+ dependents), permissively-licensed (MIT) safe-wrapper crate and these two calls are exactly its core competency, unlike the fd-3 socketpair/dup2 dance which stays on raw `libc` to match the already-proven pattern. |
| `command-group` | `5.0` (not yet in fleet's lock — greenfield addition per task brief) | Whole-process-tree supervision (kill the entire group, not just the direct child) as a typed API on top of `std::process::Command`, replacing the hand-rolled `libc::kill(-(pid), SIGKILL)` pattern at `main.rs:3174/3190` with a crate whose whole purpose is exactly this — maintained, MIT-licensed, small dependency surface. If Opus prefers keeping the hand-rolled `libc::kill(-pid, ...)` pattern (it is already proven, per §5), this dependency can be dropped; flagged as an open choice, not a hard requirement. |
| `rmcp` | `3.1` (pin to `3.1.4`, already resolved in `fleet/keel/Cargo.lock`) | The MCP tool-manifest types (`Tool`, schema helpers) this crate's `resolve_hermetic_provision` reuses from `mcp.rs`'s `ToolManifest`/`Lease` shapes — this crate constructs a manifest, it does not run the server (§2 non-goal), so it needs only `rmcp`'s model/schema types, not its `ServerHandler`/transport machinery. |
| `cedar-policy` | `4.x` (not yet in fleet's lock — greenfield addition per task brief; verify current major at build time) | Named in the task brief for `Scorecard`'s "MIGRATION-PLAN §7" re-homing context, but this blueprint does **not** find a concrete use for a policy-evaluation engine in the API surface above — `resolve_hermetic_provision`'s skill/capability resolution is a plain set-membership check (ported from `skills.rs`), not a policy language. **Flagging for Opus**: either this dependency is for a not-yet-specified future authorization layer (e.g. gating which MCP tools a lease grants beyond simple prefix-matching) and belongs in a LATER revision of this blueprint, or it is not actually needed by `fleet-worker` and the brief's dependency list is aspirational — recommend deferring this dependency until a concrete `cedar-policy` use is named, rather than adding an unused heavy dependency now. |
| `tokio` | `1.53` (pin to `1.53.1`, already resolved in `fleet/keel/Cargo.lock`) | Required transitively by `rmcp`'s model types even without running the server; if `join` grows an async variant (flagged in §6), this is also that runtime. This crate's OWN `spawn`/`join` as specified above are synchronous (`std::process::Command`-based, matching `main.rs` exactly) — `tokio` is not used to drive them directly. |
| `thiserror` | `2.0` (pin to `2.0.20`, matches `fleet-types`/`fleet-router`'s convention) | Every fallible operation returns a typed error enum (L8 rule) — `SpawnError`/`JoinError`/`ProvisionError` all derive `thiserror::Error`. |
| `serde_json` | `1.0` (pin to `1.0.151`) | fd-3 packet (de)serialization, `LaneOutcome::Done.body`, `HermeticProvision.mcp_tool_manifest`. |
| `fs2` | `0.4` (pin to `0.4.3`, already resolved) | `Scorecard`'s file-locked read-modify-write, ported verbatim from `agent.rs`. |
| `toml` | `0.8` (pin to `0.8.23`, already resolved) | Reading the `.fleet/` tree's skill/agent declaration files, same format as today's `skills.toml`/`agents.toml`. |

**Runtime (not compile-time) dependency:** `fleet-crew` (Python package at `fleet/crew/`) — invoked
as a subprocess via `python3` exactly as `run_model_agent` does today, never linked or imported as a
Rust crate. A missing/broken `crew` environment surfaces as `SpawnError::ProcessSpawnFailed` /
`LaneOutcome::EnvironmentFault`, never a compile-time failure.
...
crates/fleet-worker/
  ...
  src/
    ...
    adapter.rs        # ~35 — CliAdapter, cli_binary_name()
    request.rs        # ~40 — SpawnRequest, LaneHandle, SpawnError, JoinError (data only)
    outcome.rs         # ~45 — LaneOutcome, NoAmbientProbeResult (data only)
    spawn/
      mod.rs           # ~15 — re-exports spawn()
      worktree.rs       # ~75 — create()/remove()/unique_name() ported from worktree.rs
      process_group.rs  # ~40 — start_process_group()/wait_with_deadline()/terminate_group()
      fd3.rs            # ~70 — socketpair setup, dup2 pre_exec, recv+validate_submission
    sandbox/
      mod.rs            # ~15 — re-exports resolve_hermetic_provision()
      hermetic_env.rs    # ~55 — per-lane tempdir, HOME/XDG_* redirection, env_clear()+allowlist
      manifest.rs        # ~65 — Lease/ToolManifest ported from mcp.rs (manifest only, no server)
      skills.rs          # ~70 — Registry/Resolution ported from skills.rs, .fleet/-tree loading
    inject/
      mod.rs            # ~15 — re-exports system prompt assembly
      system_prompt.rs   # ~40 — assembles the system prompt from .fleet/ + resolved skills
    probe.rs             # ~60 — probe_no_ambient()
    scorecard.rs          # ~75 — Scorecard, scorecard_path(), record_scorecard_outcome() ported from agent.rs
  tests/
    spawn_join_happy.rs    # ~65 — successful lane end-to-end against a real stub CLI
    spawn_join_refusals.rs # ~65 — empty task, missing CLI, malformed fd-3, timeout-kill
    hermetic_isolation.rs  # ~70 — asserts spawned env never contains real HOME/XDG values
    scorecard_concurrency.rs # ~50 — concurrent record_scorecard_outcome calls don't lose updates
```
> If any file above still projects over 80 lines once bodies land, split again (e.g. `fd3.rs` →
> `fd3_send.rs` + `fd3_recv.rs`). The file-size gate (§10) is run before Opus review.
