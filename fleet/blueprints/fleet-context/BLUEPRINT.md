...
building and querying a BM25 full-text index over a document corpus (tantivy, greenfield), computing
dense embeddings for that same corpus (fastembed/ort, greenfield), fusing the two ranked lists into
one (reciprocal rank fusion, hand-rolled — see §5/§7 on why the crate literally named `rank-fusion`
is not used), and compacting the fused, ranked candidate set down to a hard token budget using real
tokenizer counts (tiktoken-rs) — greedily including what fits, and optionally asking an injected
summarizer to shrink a candidate that would otherwise be dropped. Given the same corpus, query, and
budget, `retrieve_context` returns the same `ContextSlice` every time.
...
| `fastembed` | `6.0.3` (crates.io `max_stable_version`, 3.2M downloads), **default features disabled, `hf-hub` feature left off** | Text embeddings (§3 D) via ONNX inference. New dependency. **Heavy-dep risk, flagged for Opus:** `fastembed` 6.x hard-pins `ort = "=2.0.0-rc.13"` — `ort` has published 43 versions since inception and has **never cut a stable 1.0/2.0** (`max_stable_version: null` on crates.io even at 17.7M downloads); it also links a large native ONNX Runtime binary per target platform, which is a real build-time/supply-chain weight, not just an API-churn risk. Any future `ort` breaking change requires a lockstep `fastembed` bump this crate cannot control independently. If this risk is judged too high at review, the fallback is to make `OrtEmbedder` sit behind the same seam SCIP sits behind (§2) and ship compaction/BM25 without vector search in v1 — flagged as an option, not decided here. |
...
crates/fleet-context/
  ...
  src/
    ...
    error.rs           # ~30 — ContextError
    types.rs           # ~55 — SourceFile, Language, SymbolId, SymbolRef, ScoredChunk, PlacedChunk, ContextSlice
    parse/
      mod.rs            # ~10 — re-exports
      language.rs       # ~25 — language_for (graph.rs:767-774, retyped)
      extract.rs        # ~75 — collect_nodes/definition/call/node_text (graph.rs:826-936, ported verbatim)
      symbol_id.rs      # ~30 — SymbolId::derive (blake3 hash, replaces graph.rs:938-975's uuid path)
    repomap.rs          # ~70 — build_repo_map: parse each file + resolve_edges (graph.rs:977-1035, ported) + pagerank call
    pagerank.rs         # ~65 — pagerank() power iteration, greenfield
    bm25.rs             # ~75 — TantivyIndex (open/index/search), IndexDoc, IndexLocation
    embed.rs            # ~70 — OrtEmbedder (load/embed_batch), VectorIndex trait
    fuse.rs             # ~40 — fuse_rrf, hand-rolled RRF
    tokens.rs           # ~30 — TokenModel, count_tokens (tiktoken-rs wrapper)
    compact.rs          # ~75 — Summarizer trait, compact_to_budget
    retrieve.rs         # ~55 — RetrievalQuery, retrieve_context orchestration
  tests/
    repomap_fixtures.rs     # ~65 — reuses fleet/keel/tests/graph-fixtures/* (known-callers, false-positive, known-python, known-bash)
    pagerank_properties.rs  # ~55 — mass-conservation, dangling-node, determinism
    fuse_rrf.rs             # ~50 — RRF arithmetic, tie-break, duplicate-within-ranking
    compact_budget.rs       # ~70 — first-fit-not-first-halt, summarizer fallback, budget-never-exceeded
    retrieve_pipeline.rs    # ~60 — end-to-end with fake TantivyIndex(Ram)/fake VectorIndex/fake Summarizer
```
...
tantivy = "0.26.1"
tiktoken-rs = "0.12.0"
fastembed = { version = "6.0.3", default-features = false }
thiserror = "2.0.20"
