...
for small state that needs neither a JSONL chain nor a SQL schema.
...
| `redb` | `2.2` (**new** — not in `fleet/keel/Cargo.lock` today; verify current version at implementation time with `cargo add redb --dry-run`) | Pure-Rust, single-file, ACID, MVCC embedded KV with no C dependency — fits "embedded KV where it fits" without pulling a second SQL engine in for state that's genuinely just key/value; actively maintained (cberner/redb), MIT/Apache-2.0 dual-licensed. |
...
crates/fleet-store/
  Cargo.toml
  src/
    lib.rs                 # ~20 — module decls + re-exports (IoFault + the 4 store types)
    io_fault.rs             # ~25 — IoFault enum
    lock.rs                 # ~35 — FileLock (fs2-backed), ported from main.rs:4310-4328
    ledger/
      mod.rs                 # ~10 — re-exports
      types.rs                # ~55 — LedgerPaths, LedgerError, VerifiedChain
      read.rs                  # ~45 — rows(), read_rows() helper
      append.rs                 # ~70 — append(): verify-then-extend, blake3 hash, fs write
      verify.rs                  # ~75 — verify(): the chain walk + per-row checks
    graph/
      mod.rs                  # ~10
      types.rs                 # ~65 — ProjectRecord/FileRecord/SymbolRecord/EdgeRecord/AliasRecord/ReindexBatch/DependentRecord/GraphError
      schema.rs                 # ~45 — open(): connection + CREATE TABLE/INDEX
      read.rs                    # ~55 — project(), symbols(), symbol_ids_for()
      write.rs                    # ~70 — replace_project() transaction
      dependents.rs                # ~50 — the recursive-CTE query
    memory/
      mod.rs                   # ~10
      types.rs                  # ~55 — MemoryRow/Severity/Bm25Hit/VectorHit/MemoryError
      schema.rs                  # ~55 — open(): pragmas, CREATE TABLE/FTS5/triggers, load vec0
      write.rs                    # ~60 — upsert(), set_embedding(), embedding_hashes()
      search.rs                    # ~55 — bm25_search(), vector_search(), count()
    kv/
      mod.rs                   # ~10
      types.rs                  # ~15 — KvError
      store.rs                   # ~55 — open()/get()/put()/delete()/list_prefix() via redb
  tests/
    ledger_chain.rs            # ~65 — append/rows happy path, seq/hash/prev_hash stamping
    ledger_tamper.rs             # ~75 — reorder/fork/content-mismatch each caught with the right variant
    graph_reindex.rs               # ~70 — replace_project idempotence, rollback-on-error
    graph_dependents.rs               # ~65 — recursive closure at depth 1/2/N, depth=0 rejected
    memory_search.rs                    # ~70 — upsert/bm25_search/vector_search, dimension mismatch
    kv_roundtrip.rs                       # ~45 — put/get/delete/list_prefix, absent-key semantics
```
