...
> `MIGRATION-PLAN.md` wins on crate boundary/DAG position. See `blueprints/_TEMPLATE.md` for the
> template this file adapts, and `blueprints/fleet-router/BLUEPRINT.md` for the rigor bar.
...
- Does **not** compute a routing decision — `route::command`'s arg-parsing shell stays here, but the
  6-stage filter (`decide`) is `fleet-router`'s (already blueprinted, `blueprints/fleet-router/`).
...
**Restate service shape:** the 8-stage graph is one `#[restate_sdk::service]` (stateless dispatch —
each pipeline run is independent, keyed by `TaskId`, no cross-run shared state) rather than a
`#[workflow]`/virtual object, because nothing in the pipeline needs the "callable after completion"
interaction semantics `WorkflowContext`/`SharedWorkflowContext` add — a plain `Context<'_>` handler
whose body is a sequence of `ctx.run(|| ...)` closures (one per stage) is sufficient for "journal
each stage so a crash resumes at the next one," which is the only durability property required here.
...
| Crate | Version | Why |
|---|---|---|
| `clap` | `4` (already declared, `fleet/keel/fleet/Cargo.toml:8`, `features = ["derive"]` already listed but unused) | Derive-based `Cli`/`Commands` (§3), replacing the hand-rolled `match` at `main.rs:57-252`. |
| `clap_complete` | `4` (already declared, `Cargo.toml:9`, currently unused) | Generates `fleet completions <shell>` from the derived `Cli`, replacing the hand-written `print_completions` (`main.rs:5155-5214`). |
| `figment` | `0.10.19` (verified current on crates.io, 2026-09-08) | Layered config: env vars (`FLEET_STATE`, `FLEET_ROUTE_COOLDOWNS`, etc. — today read ad hoc via `env::var` in `main.rs`/`route.rs`/`meter.rs`) over an optional config file, with one typed `Config` struct instead of scattered `env::var` calls. |
| `restate-sdk` | `0.12.0` (verified current on crates.io, 2026-09-08 — `restatedev/sdk-rust`) | Durable pipeline steps: `#[restate_sdk::service]` + `ctx.run(...)` journal each stage so a crash resumes mid-pipeline instead of restarting (§4 "Restate service shape", §6's partial-failure row). |
| `tokio` | `1` with `["io-std","macros","rt","rt-multi-thread","sync"]` (already declared, `Cargo.toml:19`, resolved to `1.53.1` in `fleet/keel/Cargo.lock` — add `rt-multi-thread`/`sync` features, not currently listed, needed for the multi-threaded runtime and `mpsc` channels) | Async IO for the pipeline's ingress adapters and the Restate service host. |
| `rayon` | `1.12.0` (verified current on crates.io, 2026-09-08 — not previously a fleet dependency) | CPU-bound stages (`Scan`, parts of `Verify`) run inside a rayon scope sized to `ConcurrencyCap`, alongside tokio for the IO-bound stages — the brief's explicit "tokio (async IO) + rayon (CPU)" split. |
| every `fleet-*` crate (§1 imports) | workspace-path (`{ path = "../crates/<name>" }`) | The actual work each pipeline stage delegates to. |

No new HTTP/serialization framework is added beyond `serde`/`serde_json` (already present) — this
layer's own code is wiring, not a service with its own wire format beyond what Restate requires.

## 8. Crate file layout

> **HARD RULE: every source file ≤ 80 lines.** Unlike `crates/*`, this lives at `src/` per
> MIGRATION-PLAN §2. `main.rs` today is 6460 lines; post-refactor it is ~20.

```
src/
  main.rs                     # ~20  — pre-parse (--print/REPL), Cli::parse(), build runtime, dispatch, exit
  cli/
    mod.rs                    # ~15  — module decls + re-exports of Cli/Commands
    root.rs                   # ~55  — Cli, Commands enum (§3)
    run_args.rs                # ~40  — RunArgs/SwarmArgs shared fields (--repo/--task/--agent/--role)
    sow_plan_args.rs             # ~45  — SowArgs, PlanArgs, RoleCheckArgs, AgentsArgs
    verify_args.rs               # ~45  — OracleArgs, AttestArgs, ContractArgs, GateArgs
    ledger_pr_args.rs             # ~40  — LedgerArgs, RollbackArgs, PrArgs
    ops_args.rs                  # ~40  — GraphArgs, ImpactArgs, McpArgs, FreezeArgs
  runtime/
    mod.rs                     # ~10  — re-exports
    tokio_rt.rs                 # ~30  — builds the multi-thread tokio Runtime
    rayon_pool.rs                # ~25  — global rayon::ThreadPoolBuilder sized by ConcurrencyCap
    concurrency_cap.rs            # ~40  — ConcurrencyCap + compute() (§3/§4/§6)
    config.rs                    # ~40  — figment-backed Config struct (env + file precedence)
  pipeline/
    mod.rs                     # ~15  — re-exports
    stage.rs                    # ~35  — PipelineStage enum + edge-table match (§4)
    event.rs                     # ~30  — PipelineOutcome, PipelineError (§3/§4)
    channels.rs                   # ~45  — blueprint_q/build_q mpsc setup, Locked-gate enqueue fn
    graph.rs                     # ~75  — the 8-stage sequential wiring, one fleet-* call per stage
    service.rs                   # ~55  — #[restate_sdk::service] impl hosting run_pipeline
  dispatch/
    mod.rs                     # ~15  — re-exports
    route_cmd.rs                  # ~35  — `fleet route`: parse -> fleet_router::decide -> print
    swarm_cmd.rs                  # ~65  — `fleet swarm ...`: parse -> pipeline::graph Dispatch stage -> print
    verify_cmd.rs                 # ~60  — oracle/adjudicate/attest/gate/contract shells -> fleet-verify
    ledger_cmd.rs                  # ~50  — ledger/rollback/pr shells -> fleet-store/fleet-merge
    ops_cmd.rs                    # ~55  — status/console/graph/impact/mcp/completions/doctor/version shells
  print/
    human.rs                     # ~40  — human-formatted table/line helpers shared across dispatch/*
    json.rs                      # ~20  — serde_json::to_string(_pretty) wrapper + error mapping
tests/
  cli_parses_every_subcommand.rs  # ~50  — table-driven: every real subcommand string parses
  concurrency_cap_bounds.rs        # ~45  — property + boundary tests for ConcurrencyCap::compute
  pipeline_resumes_after_crash.rs   # ~70  — kills the process mid-pipeline, asserts journal replay
```
> If any file above still projects over 80 lines once bodies land, split again (e.g. `graph.rs` →
> `graph_stages_1_4.rs` + `graph_stages_5_8.rs`). The file-size gate (§10) runs before Opus review.

`Cargo.toml` sketch (top-level binary crate, `fleet/keel/fleet/Cargo.toml` reshaped):
```toml
[package]
name = "fleet-cli"
version = "0.1.0"
...
restate-sdk = "0.12.0"
tokio = { version = "1", features = ["io-std", "macros", "rt", "rt-multi-thread", "sync"] }
rayon = "1.12.0"
