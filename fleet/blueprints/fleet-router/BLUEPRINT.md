...
| Crate | Version | Why |
|---|---|---|
| `serde` | `1.0` (pin to whatever `fleet/keel/fleet/Cargo.lock` already resolves — route.rs already depends on it) | `Stage`/`Decision`/`Refusal` derive `Serialize` for the `--json` output and for receipts. |
| `serde_json` | `1.0` (match fleet's lock) | Only needed by the crate's own tests/doctests if they assert on JSON shape; the composition root does the actual `to_string_pretty` call today (route.rs:456) — that call itself lives in `src/`, not here, so this may end up a dev-dependency only. |
...
crates/fleet-router/
  ...
  src/
    ...
    table.rs       # ~50 — ORDER, CandidateSpec, Tier
    role_check.rs  # ~45 — RoleCheck, RoleRefusal, evaluate_role_check
    types.rs       # ~55 — RuntimeState, Stage, Decision, Refusal (data only, derives serde)
    allow.rs       # ~35 — role_allows()
    stage.rs       # ~70 — stage() per-candidate scoring/filtering
    decide.rs      # ~65 — decide() orchestration over table + stage() + role_check
  tests/
    decide_happy.rs      # ~60 — happy-path resolution per §9
    decide_refusals.rs   # ~70 — refusal + verifier-independence cases per §9
```
> If any file above still projects over 80 lines once bodies land, split again (e.g. `stage.rs` →
> `stage_filter.rs` + `stage_score.rs`). The file-size gate (§10) is run before Opus review.
...
**Unit tests** (in `decision.rs`/`roles.rs`, `#[cfg(test)]`):
- `order_ids_are_pairwise_distinct` — asserts no two `ORDER` entries share `id` (guards the
  "duplicate" cell in §6 statically, at test time rather than at compile time).
- `role_allows_covers_every_role_exhaustively` — asserts `role_allows` compiles without a wildcard
  arm (enforced by the `match` itself — this test documents the intent by calling it for all 5
  `Role::ALL` variants and asserting each returns a bool without panicking).
- `lead_code_gate_covers_both_directions` — `evaluate_role_check` on `(Lead, diff_adds_code: true)`
  is `Err(LeadWroteCode)`; on `(Lead, diff_adds_code: false)` is `Ok(())`.
- `verifier_self_check_covers_both_directions` — same builder/verifier model → `Err(SelfVerified)`;
  distinct models → `Ok(())`.
- `decision_refusal_and_selection_are_mutually_exclusive` — for a table of representative
  `RuntimeState`s (empty, full, partial), assert `decision.refusal.is_some() == decision.selected_adapter.is_none()` always (the invariant from §4).

**Integration tests** (`tests/decide_pipeline.rs`, calling only the public API):
- `every_filter_stage_names_its_empty_set` — ported from route.rs:504-587: drive `decide` into a
...
- `verifier_compares_resolved_identity_in_both_directions` — ported from route.rs:590-611: a codex
  builder resolves the verifier to sonnet; a `"requested-codex-alias"` builder resolves it to codex
  (i.e. the comparison is against `resolved`, not `requested`).
- `lead_implementation_refuses_and_lead_general_routes_to_opus` — ported from route.rs:614-651.
- `deterministic_across_runs_and_both_availability_directions` — ported from route.rs:654-686: 20
...
- Flipping `first_empty.get_or_insert(...)` to unconditional overwrite (so a later stage's refusal
  clobbers an earlier one) must be killed by a dedicated `first_refusal_wins_over_later_stages`
  test: construct a `RuntimeState` that empties both stage 3 and stage 4, assert `refusal.stage == 3`.
...
**Property tests** (`proptest`, optional but recommended given the D50 history of a silently
unreachable candidate):
- *Every reachable candidate is reachable*: for each `id` in `ORDER`, construct a `RuntimeState`
  making exactly that candidate's adapter capable/quota'd/uncooled and every other adapter absent,
  and assert `decide` for a role whose tier contains that candidate selects it. This is the direct,
  generalized regression test for D50 (`freelane` being unconditionally unreachable) — it must
  fail loudly the moment any future `ORDER` entry becomes structurally unreachable, at N = 5 cases
  minimum (one per current entry), growing automatically as `ORDER` grows.
