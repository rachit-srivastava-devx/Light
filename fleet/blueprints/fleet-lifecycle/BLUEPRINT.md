...
crates/fleet-lifecycle/
  ...
  src/
    ...
    states.rs            # ~60 — sealed::Sealed, State, 16 marker structs, seal_states! invocation
    task_id.rs            # ~20 — TaskId, Display
    receipt.rs            # ~20 — TransitionReceipt, ReceiptLedger
    human_approval.rs      # ~20 — HumanApproval
    task.rs                # ~40 — Task<S> struct + impl<S: State> (id, retry_depth, private transition)
    edges_intake.rs        # ~30 — Task<Intake>: new, specify, refuse
    edges_specified.rs     # ~25 — Task<Specified>: review, refuse
    edges_straight.rs      # ~40 — edge! macro + its 11 invocations
    edges_refusal.rs       # ~25 — refusal_edge! macro + its 5 invocations
    edges_attested.rs      # ~15 — Task<Attested>::accept
    proposal_types.rs      # ~35 — ChangeEmitter, ProposalRequest, ProposedChange
    attestation.rs         # ~45 — AttestationBundle, REQUIRED, missing_element
    attestation_checks.rs  # ~55 — independent_verification/oracle_independence/blind_suite _is_complete, is_valid_artifact_id
    edges_propose.rs       # ~45 — Task<Accepted>::propose
    resume.rs              # ~50 — AnyTask, resume()
    advance.rs             # ~55 — advance_any() (mirrors canonical_next/typed_advance, pure)
  tests/
    ...
    happy_path.rs          # ~70 — full Intake..Observed walk, receipt count, id/retry_depth checks
    propose_gates.rs        # ~65 — attestation completeness / digest / empty-diff refusal ordering
    resume_bridge.rs         # ~45 — resume() + advance_any() over all 16 states, including Accepted's PR_EMIT_REQUIRES_EVIDENCE refusal
```
Every file above is a straight split of `lifecycle.rs`'s already-short blocks (§5's line-range
column); none requires new logic to fit under 80 lines except `resume.rs`/`advance.rs`, which are new
(§ divergence note) and sized to match.
...
**Unit tests** (co-located `#[cfg(test)]`, ported from `lifecycle.rs:1185-1478` where noted):
- `empty_evidence_refuses_without_advancing_or_writing` — ported verbatim: `Task::new(...).specify("  ", &ledger)` is `Err(EMPTY_TRANSITION_EVIDENCE)` and the `MemoryLedger` stays empty.
- `ledger_append_failure_yields_no_new_task` — a `MemoryLedger` variant whose `append` always returns `Err` makes `specify` return that `Err`; assert no `Task<Specified>` was ever produced (the compiler already prevents inspecting a value that was never returned, but the test asserts the ledger recorded nothing extra either).
- `missing_element_names_the_first_incomplete_or_absent_element` — ported verbatim (complete/pending-oracle-independence/missing-adequacy cases).
- `pinned_element_names_match_the_enforced_set` — ported verbatim, asserts `AttestationBundle::REQUIRED`'s exact 8-element array and order.
- `emitter_failure_leaves_no_receipt` — a `RecordingEmitter` variant whose `emit` returns `Err`; assert `propose` returns that `Err` and the ledger's receipt count is unchanged from before the call.

**Integration tests** (`tests/`, calling only the public API):
- `happy_path.rs::legal_path_consumes_each_state_and_records_every_edge` — ported verbatim (15 receipts, final `task.id()` check) plus asserting `retry_depth()` is unchanged across every edge (no edge silently bumps it — nothing in `lifecycle.rs` claims to, so a drift here is a regression).
- `propose_gates.rs::complete_bundle_advances_to_proposed_and_records_one_receipt` — ported verbatim.
- `propose_gates.rs::incomplete_bundle_refuses_before_the_emitter_runs` — ported verbatim.
- `propose_gates.rs::digest_mismatch_refuses_before_the_emitter_runs` — new: same shape as the incomplete-bundle test but with `request.artifact_id` set to a hash that does not match `request.diff`; asserts `ARTIFACT_DIGEST_MISMATCH` and an empty `RecordingEmitter`.
- `propose_gates.rs::empty_diff_refuses_after_digest_check_passes` — new: `request.diff = vec![]` with `artifact_id` correctly set to `blake3::hash(&[]).to_hex()` (so the digest check passes), asserts `EMPTY_DIFF` and an empty emitter — this is the one case that specifically proves emptiness is checked, not just digest.
- `resume_bridge.rs::resume_reconstructs_every_one_of_the_16_states` — for each of the 16 wire names, `resume(name, id.clone(), 0)` returns the matching `AnyTask` variant (a `match` asserting the right arm, not just `is_ok()`).
- `resume_bridge.rs::resume_refuses_an_unknown_state_name` — `resume("Bogus", id, 0)` is `Err(UNKNOWN_LIFECYCLE_STATE)`.
- `resume_bridge.rs::advance_any_walks_the_canonical_edge_from_every_resumable_state` — for each of the 15 non-`Refused` states, `advance_any` on the `resume`d task lands on the documented next state (mirrors `lifecycle.rs`'s `canonical_next` table) EXCEPT `Accepted`, which must be `Err(PR_EMIT_REQUIRES_EVIDENCE)`.
- `resume_bridge.rs::advance_any_refuses_from_refused` — `Refused` has no forward edge: `Err(ILLEGAL_LIFECYCLE_TRANSITION)`.
...
```
Expected: all unit + integration + compile-fail tests pass, 0 skipped — publish `<passed>/<total>`
(the compile-fail harness counts as 1 test regardless of its 6 cases; publish those 6 as a named
sub-count too, e.g. "unit 5/5, integration 9/9, compile-fail 6/6 cases, trybuild harness 1/1").
