...
crates/fleet-events/
  ...
  src/
    ...
    ids.rs             # ~30 — EventId + derive()/as_blake3()
    envelope.rs         # ~50 — SourceKind, EventKind (+ side_effecting), EventEnvelope, MAX_PAYLOAD_BYTES
    guard.rs            # ~55 — GuardedAction, MAX_QUOTE_BYTES, guard()
    clock.rs            # ~20 — Clock trait + SystemClock
    sink.rs             # ~25 — SinkError, EventSink trait
    adapter.rs           # ~20 — Adapter trait
    ingest.rs            # ~65 — IngestError, IngestReport, ingest_once()
    adapters/
      mod.rs             # ~10 — re-exports
      github.rs          # ~75 — GithubAdapter: reqwest poller, cursor = last delivery id/ETag, HMAC check
      gmail.rs            # ~75 — GmailAdapter: imap poller, cursor = last UID, mailparse body extraction
      fs_watch.rs          # ~75 — FsAdapter: notify bounded-timeout poll, cursor = last snapshot
      cli.rs               # ~35 — CliAdapter: wraps a single pre-read argv/stdin invocation
  tests/
    id_derivation.rs        # ~40 — EventId determinism/collision-avoidance
    guard_behavior.rs         # ~70 — guard()'s full §6 table
    ingest_orchestration.rs    # ~75 — ingest_once() with fake Adapter + fake EventSink
    adapters_cli.rs             # ~40 — CliAdapter end-to-end (no network/fs needed)
```
...
1. **`fleet-store` has no blueprint yet (state `todo`, §5 of MIGRATION-PLAN, confirmed empty
   `blueprints/fleet-store/` on disk 2026-09-08) — so this blueprint cannot cite a real
