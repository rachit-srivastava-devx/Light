...
| `thiserror` | `2.0.20` (matches `fleet/keel/Cargo.lock`'s already-resolved version) | `ZeroDenominator`'s typed error. |

No subprocess/async/regex crate is a *required* dependency of the library itself — `parse_denominator`
implementations may use `regex` internally (`fleet/keel/Cargo.lock` does not currently resolve one;
if the parsers need it, add `regex` as a normal dependency and note the new resolution in the PR,
per PLAYBOOK.md rule 1 — plain `str::find`/`split` may suffice for every marker in §5's reuse map and
is preferred if so, to keep this crate's dependency footprint minimal like `fleet-router`'s).
...
crates/fleet-verify/
  ...
  src/
    ...
    requirement.rs       # ~20 — Requirement, ProbeTool
    ports.rs             # ~30 — ToolProbe, ProcessRunner, ProcessOutput (the two injected boundaries)
    denominator.rs        # ~40 — Denominator, ZeroDenominator, DenominatorResult
    spec.rs                # ~25 — GateSpec
    verdict.rs              # ~45 — FailReason, Verdict, GateResult
    report.rs                # ~50 — Report + passed/failed/skipped/env_faults/exit_code
    classify.rs               # ~45 — classify() (private; the invariant-enforcing fn)
    orchestrate.rs             # ~35 — run_gate, run_all (public entry points)
    parsers.rs                  # ~75 — one small fn per gate family (caught/total, scanned/findings,
                                 #        checked/flagged, libtest passed/failed, N/N); each ~5-8 lines
    registry.rs                  # ~70 — const GATES: &[GateSpec] = &[...]; the committed 24-gate table
  tests/
    denominator_invariant.rs     # ~35 — Denominator::new rejects total==0; accepts total>0 incl. numerator==0
    classify_rules.rs            # ~70 — every classify() branch in §6's table, via fake ProcessOutput values
    orchestration_probe_skip.rs  # ~60 — run_gate/run_all with a fake ToolProbe: required-missing → env
                                  #        fault counted; advisory-missing → skip, no env fault
    registry_integrity.rs         # ~30 — GATES ids pairwise distinct; every id has a non-empty command
    parsers_table.rs               # ~75 — one fixture per real gate script's stdout (copied verbatim
